...
//!
//! Exercises the full 3-node warden cluster: raft consensus, workload deploy
...
//!   see yah-warden-integration-testing.md §KNOWN-LOCAL-GAPS.
...
///
/// A 3-node warden cluster is used throughout (raft quorum = 2/3). The test
...
///
/// Local tier: all nodes are in-process warden servers connected by real
    ...
    R: ContainerRuntime + 'static,
{
    ...
    let state_url = format!(
        "{}/workloads/{}/state",
        cluster.warden(leader_idx).base_url,
