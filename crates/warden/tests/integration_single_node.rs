...
//!
//! Exercises the full warden HTTP API → `ContainerRuntime` path for one node:
//! start warden in-process, deploy a `WorkloadSpec`, wait for Running,
...
//! cargo test -p warden --features containerd-integration \
...
//! @yah:next("Human verify: cargo test -p warden --features containerd-integration --test integration_single_node against a Colima socket. Confirm single_node_e2e__local passes. Then archive F10 and proceed to verify F6+F7+F8+F9 the same way.")
...
//! @yah:verify("cargo test -p warden --features containerd-integration --test integration_single_node — single_node_e2e__local passes deploy+state round-trip against Colima containerd. Smoke variant excluded without YAH_SMOKE=1.")

use std::time::Duration;

use cloud::provider::MachineProvider;
use constable_core::Constable as ContainerRuntime;
use warden_test_harness::{test_cluster, wait_for_state, WorkloadStatus};
...
/// Single-node E2E: start warden, deploy a `WorkloadSpec`, assert it reaches
/// `Running`, then verify state round-trips via the `/workloads/{ident}/state`
/// RPC.
///
/// Local tier: warden runs in-process with `ContainerdRuntime` pointing at
/// the Colima socket (or the system containerd socket on Linux). The workload
/// is an `alpine` container deployed into containerd.
///
/// Smoke tier: warden runs on a real Hetzner CPX-11 machine provisioned via
    ...
    R: ContainerRuntime + 'static,
{
    ...
    let warden = cluster.warden(0);

    // Sanity: warden is healthy before we try to deploy.
    warden
        .health_check()
        .await
        .expect("warden /health should return 200 before deploy");
