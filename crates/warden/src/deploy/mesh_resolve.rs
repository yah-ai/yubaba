//! Deploy-time mesh-address resolution for warden workloads (R090-F6).
//!
...
//!    which ports they expose", abstracted so warden's production impl reads
...
//! layer; only becomes a literal when warden assembles the containerd spec
...
///
/// `dependency_wait_deadline` is the time warden should be willing to wait
...
///
/// Warden's production impl reads from raft; tests use an in-memory fake.
...
// ── In-memory state used by tests and by warden's pre-raft single-node mode ──

/// Simple `HashMap`-backed [`MeshState`] for tests and for warden's
/// not-yet-on-raft single-machine mode.
///
/// The production multi-machine warden replaces this with a raft-state read
/// in `crates/yah/warden/src/raft/`.
