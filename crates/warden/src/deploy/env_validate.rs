//! Deploy-time environment validation for warden workloads.
//!
...
//!
//! **No implicit retries.** Warden's `RestartPolicy` machinery decides whether
//! and how to retry on failure.
//!
//! Failures surface in `warden.workloads_status` RPC and are logged to
...
/// the journald one-line format warden logs today so scryer migration (R093) is
