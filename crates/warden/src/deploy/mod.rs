//! @yah:relay(R278, "Cross-cutter — Rollout API + gate evaluator (warden /v1/rollouts)")
//! @yah:assignee(bundle-anthropic-miravel)
//! @yah:at(2026-06-01T02:26:21Z)
//! @yah:status(in-progress)
//! @yah:parent(Q273)
//! @yah:next("F1: POST /v1/rollouts with linear strategy + staging→prod (v1 slice per yah-warden-ci-cd.md)")
...
//! @arch:see(.yah/docs/working/W140-yah-warden-ci-cd.md)
//! @yah:handoff("PrometheusGateEvaluator in warden/src/rollout/gate.rs. Queries /api/v1/query (instant). Metric name → PromQL template mapping for http_5xx_rate + p95_latency_ms; custom metrics pass through verbatim. Condition parsing: < <= > >=. 9 unit tests all green.")
