//! yah-warden: per-machine infrastructure daemon for yah-managed mirrors.
//!
...
//! R040-F7 and a follow-on warden ticket; this build wires the routes only as
...
//!
//! @yah:ticket(R040-F20, "Phase 2 — warden openraft coordination layer")
...
//! @yah:next("Library choice: openraft (databendlabs/openraft). NOT TiKV — TiKV is a distributed KV server with PD + multi-raft sharding designed for TBs / 100+ nodes; warden's coordination state is KB-scale across <10 nodes. openraft is a Rust library that compiles into the warden binary; warden owns the state machine + storage + transport.")
//! @yah:next("State machine scope: cluster membership (warden peers, current leader), service placement (which machine runs Headscale/Postgres/etc), distributed locks (in-progress provisions, mesh migrations), floating ingress claim (who currently owns the public Headscale tunnel). Headscale's own DB does NOT live here — that stays in Headscale-managed SQLite (R040-F21 handles via litestream).")
//! @yah:next("Transport: warden raft uses Tailscale-mesh IPs for peer-to-peer raft RPC. Means raft itself doesn't need Cloudflare ingress; mesh must be up before raft works (chicken-and-egg solved because mesh exists by Phase 2 — Phase 1a/1b already up).")
...
//! @yah:next("Surface: warden raft {status,peers,transfer-leader} subcommand. yah mesh status (R040-F18) gains a raft-state line once this lands.")
...
//! @yah:verify("warden raft status on a 3-node cluster shows leader + 2 followers; transfer-leader moves leadership; killing the leader elects a new one within seconds")
...
//!
//! @yah:ticket(R040-F21, "Phase 2 — Headscale floating coordinator: warden-orchestrated + litestream-replicated")
//! @yah:at(2026-05-05T00:29:06Z)
//! @yah:assignee(agent:claude)
//! @yah:status(review)
//! @yah:phase(P3)
//! @yah:parent(R040)
//! @yah:next("Implements the floating-coordinator part of Phase 2 from .yah/docs/architecture/A041-yah-mesh-bootstrap.md. Hard-depends on R040-F20 (warden openraft layer) for leader election + service placement.")
//! @yah:next("Headscale state replication: litestream sidecar streams the SQLite WAL to Hetzner Object Storage (and optionally to peer warden nodes). On leader change, the new leader runs litestream restore (seconds for small DB) before starting Headscale. Worst-case staleness on failover bounded by litestream flush cadence (~1s default).")
//! @yah:next("Service-orchestration: warden runs Headscale + litestream as managed systemd units ONLY on the raft leader. Followers pre-warm their litestream restore by pulling periodic snapshots so promotion latency is bounded.")
//! @yah:next("Floating ingress (HTTPS only): each warden node runs a cloudflared replica connected to the mesh.<your-domain> named tunnel. All replicas serve a healthcheck endpoint that returns 200 only if 'I am the raft leader AND my Headscale is healthy', else 503. Cloudflare load-balances among 200-replicas, so leader change = ~10s ingress flip with no DNS or IP changes.")
...
//! @yah:next("Open: litestream destination — S3 alone vs S3 + peer warden nodes. Start with S3-only; add peer replication if S3 outage scenarios warrant the extra complexity.")
//! @yah:verify("3-node cluster with warden raft + Headscale running on leader; killing the leader's machine triggers election + Headscale restore on new leader within ~30s; new node provision succeeds against the new leader using the unchanged mesh.<your-domain> URL")
...
//! @yah:status(review)
//! @yah:parent(R278)
//! @yah:next("Add rollout module to warden (src/rollout/) with RolloutStore + RolloutRecord types")
...
//! @yah:handoff("POST /v1/rollouts (202), GET /v1/rollouts, GET /v1/rollouts/{id}, POST /v1/rollouts/{id}/override all wired in lib.rs. RolloutEngine background task spawned on create. RolloutStore + RolloutRecord + RolloutStatus types in warden/src/rollout/mod.rs. 6 new handler tests green (create, reject-non-linear, get, get-404, override-promote, list). YAH_PROMETHEUS_URL env + with_prometheus_url() builder wired.")
//!
//! @yah:relay(R406, "Core: Warden/Constable split + native driver + runtime parity")
...
//!
//! @yah:ticket(R406-T8, "Warden-proper extraction: remove in-process supervision, dispatch to Constable over UDS")
//! @yah:assignee(agent:claude)
...
//! @yah:handoff("WARDEN-SIDE CONSTABLE DISPATCH LANDED. New crate dep + module crates/yah/warden/src/constable_client.rs ConstableClient owns a persistent UnixStream (tokio::net), serial-mutex'd via Inner { rd, wr, buf }. Surface: connect()/connect_with_timeout() runs Hello→Welcome handshake (captures ConstableInfo{version, constable_version}); list()/stop()/drain(budget)/probe() each allocate a fresh RequestId, write a postcard frame, await one ConstableToWarden reply, and check the rid matches. Remote Error{code,message} surfaces as ClientError::Remote so warden HTTP handlers can branch on category. ServerState gained constable_client: Option<Arc<ConstableClient>> + with_constable_client() builder; yah-warden serve grew --constable-socket <path> with attach_constable_client() that warns + falls back to in-process runtime on connect failure (5s timeout — short enough not to stall systemd, long enough to ride the constable.service unit settling).")
//! @yah:handoff("HANDLERS WIRED TO PREFER CONSTABLE. GET /workloads, GET /workloads/{id}/state, POST /workloads/drain all check s.constable_client first and dispatch through the UDS when set; legacy s.runtime is the fallback. New response header x-workload-source: constable|runtime|stub lets callers branch on row shape (constable returns WorkloadEntry {id,state,pid}; runtime returns the existing rich WorkloadState). drain_workloads sends a structured Drain{flush_ms=5000, checkpoint_ms=1000} per workload — matches the parity floor in W154 §Runtime parity contract. POST /workloads/deploy is INTENTIONALLY still on the legacy path: Constable's Deploy arm returns Error{Internal, 'backend driver not implemented (R406-T4..T6/T11)'}; routing deploy through it before R406-T9 would break every single-node and clustered deploy. The handler doc-comment documents this explicitly.")
//! @yah:handoff("TESTS GREEN. 6 unit tests in constable_client::tests (handshake, list, stop, drain ack, remote-error, connect-timeout) using one-shot in-process UDS servers. 3 new integration tests in crates/yah/warden/tests/integration_constable_client.rs that spawn constable::serve_with_shutdown against a tempdir socket and round-trip handshake+list, drain-unknown, stop-unknown via the real warden client (no mocks on either side — proves wire compatibility). Full suite: 92 warden lib + 3 new warden integration + 47 constable + 19 constable-proto pass. cargo check -p warden --features containerd-integration also clean — no feature-flag regression.")
//! @yah:next("R406-T9 follow-on: once Constable's Deploy arm is wired (containerd backend), migrate POST /workloads/deploy in crates/yah/warden/src/lib.rs to dispatch via client.deploy(id, Workload::Container(spec)) — the validation + signature check + mesh-IP allocation + cloudflare ingress + headscale operator-bridge stays in warden (they are admission decisions, not supervision). The natural ordering inside the handler is: validate → allocate ident+mesh_ip → constable.deploy → on Ok, run cloudflare/headscale registration → on registration failure, constable.stop(id) for cleanup → respond. Tests-side: integration_smoke_filter / integration_single_node / integration_public_ingress currently use the legacy runtime; either give them a constable fixture or keep them on the runtime path and add a parallel constable-fixture suite. Pick whichever is less churn at T9 time.")
//! @yah:next("ConstableClient currently uses a request/response Mutex + serial. Constable will eventually push WorkloadStarted/WorkloadExited/DrainCompleted events (see ConstableToWarden::DrainCompleted rustdoc) — at that point the client needs a background reader that demuxes responses from pushes by RequestId. The public API stays the same; the change is internal to constable_client.rs. Track as a separate sub-ticket once Constable grows the push channel; not in scope for T8.")
//! @yah:next("Deploy/get-state JSON shape currently changes when --constable-socket is set (WorkloadEntry vs WorkloadState). The x-workload-source response header is the explicit branch signal. If desktop/CLI consumers need a unified shape, the migration path is to enrich Constable's WorkloadEntry with mesh_ip+container_id once they live on Constable (T9). Not a T8 blocker.")
//! @yah:next("Try the binary end-to-end manually: in one shell, `cargo run -p constable -- --socket /tmp/constable.sock`. In another, `cargo run -p warden --features containerd-integration -- serve --constable-socket /tmp/constable.sock --bind 127.0.0.1:7443`. `curl http://127.0.0.1:7443/workloads` should return `{ workloads: [] }` with header x-workload-source: constable.")
//! @yah:verify("cargo test -p warden --lib  # 92 passed")
//! @yah:verify("cargo test -p warden --test integration_constable_client  # 3 passed")
//! @yah:verify("cargo test -p constable -p constable-proto  # 47+19 passed")
//! @yah:verify("cargo check -p warden && cargo check -p warden --features containerd-integration  # both clean")
//! @yah:gotcha("x-workload-source response header is the branch signal — constable, runtime, or stub. Don't assume row shape from URL alone; constable mode returns WorkloadEntry {id,state,pid} while runtime mode returns the rich WorkloadState {ident, container_id, status, mesh_ip}.")
//! @yah:gotcha("POST /workloads/deploy is still on the legacy in-process ContainerRuntime path on purpose. Constable's Deploy arm returns Error{Internal, 'backend driver not implemented (R406-T4..T6/T11)'} — switching deploy to the UDS now would break every deploy until R406-T9 lands the containerd backend.")
//!
//! @yah:ticket(R406-T9, "Containerd backend via Constable: route containerd RPC through Constable's WorkloadSpec enforcement")
//! @yah:assignee(agent:claude)
...
//! @yah:handoff("CONSTABLE'S CONTAINERD BACKEND LANDED. New module app/yah/constable/src/containerd.rs (Linux-feature-gated under containerd-integration) owns a tonic Channel to a containerd socket and exposes deploy(id, &WorkloadSpec) -> u32 pid, teardown(id) (idempotent kill+delete), list() -> Vec<WorkloadEntry>, and health() -> version. WorkloadSpec enforcement at dispatch is centralized in two pure functions: validate_spec_for_constable() rejects unresolved EnvValue::FromSecret / FromMesh (warden's admission must pre-resolve), and build_oci_spec() applies default-drop capabilities (CAP_NET_BIND_SERVICE only), noNewPrivileges=true, rlimit NOFILE=1024, /yah/<name> cgroupsPath, and PID/network/IPC/UTS/mount namespace isolation. Labels stamped on every container — yah.ident / yah.name / yah.tier — let reconciliation rediscover yah workloads vs other orchestrators sharing the containerd socket. Container ID is the WorkloadId.as_str() so warden's id round-trips through containerd for free.")
//! @yah:handoff("DISPATCH WIRED IN handle_message. Added ServerCtx { registry: Arc<Mutex<Registry>>, containerd: Option<Arc<ContainerdBackend>> } as the new dispatch context. handle_message takes &Arc<ServerCtx>; Deploy { Workload::Container(spec) } routes through the backend with feature-gated arms — without containerd-integration it returns BackendRefused with a clear 'rebuild with --features containerd-integration'. With the feature on but no --containerd-socket, ServerCtx.containerd is None and we return BackendRefused naming the flag. Deploy { Workload::MesofactStatic | Almanac } returns InvalidSpec — those live in warden's reconcilers, not constable. Stop dispatches teardown when the backend is attached; with no backend it acks regardless (Stop is idempotent — absence of the workload satisfies the requested end-state). List merges the in-memory registry (native workloads via R406-T5/T6) with containerd's container list when the backend is attached. Probe still returns Internal with a clear pointer at R406-T11 (probe protocol decision is its own ticket).")
//! @yah:handoff("BINARY WIRED. app/yah/constable/src/main.rs gained --containerd-socket / $CONTAINERD_SOCK; serve_with_ctx now drives the dispatch loop with a ServerCtx assembled at startup. Connect failure at startup is fatal (an operator asking for containerd should know immediately, not on the first deploy). Without the flag, warns that Deploy { Container } will refuse — sets clear expectations for the no-backend case. The non-containerd-integration build refuses --containerd-socket with a clean error and shifts dispatch into the no-backend branch.")
//! @yah:handoff("TESTS GREEN. constable lib: 49 without the feature, 58 with — new tests cover Container dispatch (with/without feature, without attached backend), MesofactStatic rejection, Stop idempotency, and 9 new pure-function backend tests (image_ref tag/digest, validate FromSecret/FromMesh, OCI cap allowlist, env-literal-only filter, label stamping, task-status mapping). Updated R406-T2/T6's uds_skeleton.rs integration test to expect Ack from Stop (was asserting the now-removed 'R406-T4' placeholder error). 1 uds_skeleton integration test passes both with and without the feature. Warden's tests/integration_constable_client.rs: 3 tests pass, including the updated stop_unknown_workload_returns_ack_for_idempotency. Full sweep: 92 warden lib + 3 warden integration + 49/58 constable lib + 19 constable-proto pass; cargo check clean in both feature modes for both crates.")
//! @yah:next("MIGRATE warden's POST /workloads/deploy through ConstableClient.deploy(). The wire payload is constable_proto::WardenToConstable::Deploy { request_id, id, spec: Workload::Container(spec) }. The migration order inside the handler: validate (already done) → allocate ident+mesh_ip (already done) → resolve EnvValue::FromMesh / FromSecret on warden's side (admission) → constable.deploy(id, Workload::Container(enriched_spec)) → on Ack, run cloudflared + headscale registration → on registration failure, constable.stop(id) for cleanup → respond. Add a deploy() method on warden's ConstableClient that sends WardenToConstable::Deploy and awaits ConstableToWarden::Ack { kind: Deploy } | Error. This was deferred from R406-T8; T9 now unblocks it.")
//! @yah:next("Once warden's deploy path migrates, run end-to-end against a real containerd (Colima on macOS or a Hetzner CPX-11). The cloud-tier flow is: `yah cloud machine provision yah-cloud-1`; ensure release.yml ships constable alongside warden; `yah cloud workload deploy warden-probe --machine yah-cloud-1`. Verify constable logs show 'containerd backend attached' and `curl http://<warden>:7443/workloads` returns the container with x-workload-source: constable (R406-T8's header).")
...
//! @yah:next("R406-T13 (systemd unit ship path) — once warden.service + constable.service ship together via release.yml, both daemons start on a fresh provision; the constable.service unit needs ExecStart=constable --containerd-socket /run/containerd/containerd.sock (or set CONTAINERD_SOCK in EnvironmentFile).")
//! @yah:verify("cargo test -p constable                                  # 49 passed (+ 1 uds)")
//! @yah:verify("cargo test -p constable --features containerd-integration # 58 passed (+ 1 uds)")
//! @yah:verify("cargo test -p warden                                     # 92 lib + 3 integration_constable_client")
//! @yah:verify("cargo check -p constable --features containerd-integration && cargo check -p warden --features containerd-integration  # both clean")
//! @yah:verify("cargo run -q -p constable -- --help                       # mentions --containerd-socket")
//! @yah:gotcha("Container ID == WorkloadId.as_str(). If warden ever sends two Deploys with the same id, the second tears down the first (idempotent redeploy by design). Stable across restarts so reconciliation can rediscover workloads via the yah.ident label.")
//! @yah:gotcha("ContainerdBackend::deploy fails before container creation if the image isn't already in containerd's image store — pulling is warden admission's job (or pre-pulled at machine provision per R040-F11). The error surfaces as BackendError::Containerd ('image not found in containerd: <ref> — pre-pull required').")
//! @yah:gotcha("Constable's default capability allowlist is intentionally tighter than warden's old runtime::containerd impl: CAP_NET_BIND_SERVICE only (drops CAP_KILL). Workloads that send signals to non-self pids will need an explicit cap field in WorkloadSpec — not currently in the type. Track as a follow-up if a real workload trips this.")
//! @yah:gotcha("Stop without a containerd backend acks anything — this differs from R406-T8's warden-side expectations which expected a 'backend not implemented' error. The integration test in tests/uds_skeleton.rs and warden's tests/integration_constable_client.rs were updated to expect Ack.")
//!
...
//! @yah:phase(P2)
//! @yah:parent(R482)
//! @yah:next("Cluster-of-one raft init per W197 §'Single-node raft'. No peers, no leader election complexity. Future multi-machine join uses warden's existing join-by-NodeId flow.")
...
//! @yah:verify("cargo test -p warden --test bootstrap_single_node")
//! @arch:see(.yah/docs/working/W197-camp-bootstraps-warden.md)
//! @arch:see(.yah/docs/architecture/A032-yah-cluster-mesh.md)
//!
//! @yah:ticket(R484-T5, "Rewire warden + app/yah/constable binary to depend on crates/yah/constable; delete warden/src/runtime/")
//! @yah:assignee(agent:claude)
...
//! @yah:handoff("Warden has been fully rewired off the constable-core shims. Deleted: crates/yah/warden/src/runtime/{containerd,docker,fake}.rs (~2.4k LoC of dead-since-T2 duplicates the shim layer was hiding) and crates/yah/warden/src/constable_client.rs (the T4 re-export shim). Migrated all warden call sites and integration tests: lib.rs `use constable_client::ConstableClient` → `use constable_core::sibling::ConstableClient`; `use runtime::ContainerRuntime` → `use constable_core::Constable as ContainerRuntime`; main.rs `warden::runtime::containerd::ContainerdRuntime` → `constable_core::containerd::ContainerdRuntime`; `warden::constable_client::connect_with_timeout` → `constable_core::sibling::connect_with_timeout`; six warden integration tests rewritten to import constable_core directly.")
//! @yah:handoff("warden-test-harness (which the integration tests pull in) followed the same path — `warden::runtime::{WorkloadState,WorkloadStatus,ContainerRuntime}` → `constable_core::*`; added `constable-core = { path = \"../constable\" }` to its Cargo.toml. warden-test-macros's proc-macro now emits `::constable_core::containerd::ContainerdRuntime` instead of `warden::runtime::containerd::ContainerdRuntime` in the generated `__local` variant; the smoke variant still references `warden::runtime::DummyRuntime` because DummyRuntime correctly stays in warden (it's warden-smoke-tier specific).")
//! @yah:handoff("crates/yah/warden/src/runtime/mod.rs was kept (not deleted) for two reasons: (1) DummyRuntime lives there and is referenced by the smoke-test macro path; (2) the file is the source-of-truth line anchor for several review-status tickets (R091-F1, R256-F10, R471-T2, R471-F3, R484, R484-T2, R484-T3) plus R484-T4 which was relocated from the deleted constable_client.rs into this file. The `pub use constable_core::*` shim re-exports were removed; only the DummyRuntime impl and the historical annotations remain. The file's module doc was rewritten to reflect its new minimal role.")
//! @yah:handoff("app/yah/constable binary intentionally NOT rewired to import constable-core. The binary is the SERVER side of the W154 UDS protocol — it accepts constable-proto frames and dispatches to its own containerd/native/cgroup/pidfd impls. The Constable trait in constable-core is the CLIENT-facing contract; ConstableClient (in constable-core::sibling) talks to this binary over UDS. Both crates already depend on constable-proto for the wire format; that IS the wiring. Reshaping the binary to internally use constable-core::Constable trait dispatch would be a substantial refactor (it'd need to convert constable-proto's WorkloadState wire enum to constable-core's WorkloadState struct on every call) and is out of scope here.")
//! @yah:handoff("Verified: cargo check -p constable-core --all-features -p warden -p warden-test-harness -p warden-test-macros -p constable -p constable-proto --all-targets → clean. cargo test -p constable-core --all-features --lib → 61 passed. cargo test -p warden --lib --features containerd-integration,testing,docker-integration → 102 passed. cargo test -p warden --test integration_constable_client → 3 passed. cargo test -p constable --all-features → lib+integration green. Pre-existing build errors in scryer/task/qed-gha (missing .await on opaque Future types) are unchanged — they exist on the parent commit and are unrelated to T5.")
//! @yah:next("Sign-off check: skim crates/yah/warden/src/runtime/mod.rs (annotations + DummyRuntime only — no shim re-exports left), grep for `constable_core::` across warden's src + tests to confirm the migration is complete, then archive R484-T5.")
//! @yah:next("T6 (desktop adopts inlined constable): app/yah/desktop adds constable-core dep, calls BackendAvailability::probe() at startup, uses Inlined::pick(&availability, &[Backend::Docker], factory) to construct an Arc<dyn Constable> backed by DockerRuntime, holds it on state. Drops any direct docker/containerd references from desktop's existing code paths.")
...
//! @yah:verify("cargo check -p constable-core -p warden -p warden-test-harness -p warden-test-macros -p constable -p constable-proto --all-features --all-targets  # clean (modulo pre-existing scryer/task/qed-gha errors)")
//! @yah:verify("cargo test -p constable-core --all-features --lib  # 61 passed")
...
//! @yah:verify("cargo test -p constable --all-features  # green")
//! @yah:verify("test ! -f crates/yah/warden/src/constable_client.rs  # shim deleted")
//! @yah:verify("test ! -f crates/yah/warden/src/runtime/containerd.rs  # orphaned dup deleted")
//! @yah:verify("test ! -f crates/yah/warden/src/runtime/docker.rs  # orphaned dup deleted")
//! @yah:verify("test ! -f crates/yah/warden/src/runtime/fake.rs  # orphaned dup deleted")
//! @yah:verify("! grep -r 'warden::runtime::ContainerRuntime\\|warden::runtime::containerd::\\|warden::runtime::fake::\\|warden::runtime::docker::\\|warden::constable_client::' crates/yah/warden/src/ crates/yah/warden/tests/ crates/yah/warden-test-harness/src/ app/yah/  # no remaining shim refs in source")
...
use constable_core::sibling::ConstableClient;
use constable_core::Constable as ContainerRuntime;

/// How warden exposes workloads with `expose.operator` set.
///
...
pub const DEFAULT_STATE_PATH: &str = "/etc/yah-cloud/identity.json";
/// Default directory for headscale binary + state when deployed via `promote`.
pub const DEFAULT_HEADSCALE_DIR: &str = "/etc/yah-cloud/headscale";
...
pub struct ServerState {
    ...
    /// lines from the same warden process can be correlated across restarts.
    ...
    /// once T9 ships Constable's containerd backend. Read handlers
    /// (`list`/`get_state`/`drain`) already prefer Constable when it is
    /// configured and fall back to this field when it isn't.
    pub runtime: Option<Arc<dyn ContainerRuntime + Send + Sync>>,

    /// UDS client for talking to Constable — `app/yah/constable`'s sibling
    /// process supervisor (R406-T8). Wired by the caller via
    /// [`Self::with_constable_client`]; `None` outside cloud-tier deploys
    /// that run the constable.service systemd unit. Workload-lifecycle
    ...
    /// `warden install` service-principal bootstrap. When set, the workload
    ...
    ///
    /// When `Some`, warden POSTs to `<cloudflared_url>/v1/tunnels` for workloads
    /// with `expose.public` set. When `None`, warden logs a warning and falls
    ...
    ///
    /// When `Some`, warden POSTs to `<headscale_url>/api/v1/preauthkey` for
    ...
    /// R374-F3: docker-CLI runtime warden uses to drive the MinIO half of
...
impl ServerState {
    ...
    /// R374-F4: register the docker-CLI runtime warden uses to drive the
    ...
    /// [`local_driver::LocalRuntime`] to warden once at startup.
    ...
    /// Attach a Constable UDS client (R406-T8). When set, workload-lifecycle
    ...
    /// `warden install` (W159 §Service principals). Without this client,
    /// warden still provisions workloads, but ownership writes are skipped
    ...
    /// `CloudflaredMock` instance. When not set, warden falls back to
    ...
    /// `HeadscaleMock` instance. When not set, warden checks
...
pub fn build_router(state: Arc<ServerState>) -> Router {
    Router::new()
        ...
        // R092-F5: WorkloadSpec deploy via warden RPC (operator-signed)
...
pub async fn serve(addr: &str, state: Arc<ServerState>) -> Result<()> {
    ...
    tracing::info!(addr = ?local, "yah-warden listening");
...
/// (R374-F2: camp writes the port to `.yah/jit/warden-pond-port.json`).
pub async fn serve_on_listener(
    listener: tokio::net::TcpListener,
    state: Arc<ServerState>,
) -> Result<()> {
    let local = listener.local_addr().ok();
    tracing::info!(addr = ?local, "yah-warden listening (embedded)");
    axum::serve(listener, build_router(state))
        .await
        .context("axum::serve")
}
...
///   - This warden node is the current raft leader.
...
///
/// R406-T8: when a Constable client is configured, dispatch through the UDS
/// instead of the legacy in-process `ContainerRuntime`. Each row is the
/// `WorkloadEntry` shape from `constable-proto` — `{ id, state, pid }`.
/// A response header `x-workload-source` is set to `constable`, `runtime`,
...
/// guarantees matter.
///
/// R406-T8: when a Constable client is configured, this is implemented as
/// `client.list()` filtered by id. The protocol's `Probe` is reserved for
/// readiness, not lifecycle introspection; until Constable gains a per-id
/// query (T9), filtering the list is the right primitive — Constable's
...
async fn get_workload_state(
    ...
    if let Some(client) = &s.constable_client {
        ...
        let id = constable_proto::WorkloadId::new(ident);
        return match client.list().await {
            Ok(entries) => {
...
///
/// R406-T8: when a Constable client is configured, list workloads via UDS
...
/// floor documented in W154 §"Runtime parity contract"). When Constable
...
async fn drain_workloads(State(s): State<Arc<ServerState>>) -> impl IntoResponse {
    if let Some(client) = &s.constable_client {
        let budget = constable_proto::DrainBudget {
            flush_ms: 5_000,
            checkpoint_ms: 1_000,
        };
...
/// `ContainerRuntime`.** Constable's `Deploy` arm in `app/yah/constable/
/// src/server.rs::handle_message` returns `Error { Internal, "backend driver
/// not implemented (R406-T4..T6/T11)" }` — wiring the warden deploy handler
/// through `client.deploy()` before Constable's containerd backend exists
...
/// registration and mesh-IP allocation stay in warden after the migration —
...
///
/// Real SSE streaming is wired in R091-F1 (warden SSE endpoint) with the
...
async fn get_workload_logs(
    ...
    (
        ...
        Json(serde_json::json!({
            ...
            "hint": "R091-F1 ships warden SSE; R093-P1 ships scryer.tail over operator-bridge",
        })),
    )
}
...
async fn headscale_deploy(
    ...
    let dl_url = headscale_linux_download_url(&req.headscale_version);
    ...
    // Write unit file — best-effort on non-systemd environments (tests, Mac).
    let _ = std::fs::write("/etc/systemd/system/headscale.service", &unit);

    // Enable + start — best-effort: non-systemd hosts return an error which is
    // surfaced to the operator but doesn't fail the file-transfer part.
    let _ = std::process::Command::new("systemctl").args(["daemon-reload"]).status();
    let svc_ok = std::process::Command::new("systemctl")
...
/// Warden runs on Hetzner Linux x86_64 servers in Phase 1.
...
async fn create_rollout(
    ...
    if req.policy.strategy != RolloutStrategy::Linear {
        return (
            StatusCode::UNPROCESSABLE_ENTITY,
            Json(serde_json::json!({
                "error": "only strategy='linear' is supported in warden v1",
...
/// `GET /v1/rollouts` — list all rollouts on this warden node, newest first.
...
mod tests {
    ...
    async fn identity_auto_generates_on_startup() {
        // R092-F8: warden generates its own Ed25519 hostkey on first boot;
    ...
    fn test_cheers_client(issuer_url: &str) -> Arc<CheersClient> {
        ...
        let cfg = cheers_client::CheersConfig {
            ...
            principal_id: "warden-test".into(),
            kid: "warden-test-1".into(),
        };
