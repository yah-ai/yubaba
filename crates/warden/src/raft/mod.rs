//! Warden openraft coordination layer — Phase 2 (R040-F20).
//!
//! Builds on top of [`openraft`] to give the warden cluster consensus
//! on:
//! - Cluster membership (who is a warden peer, current leader)
...
//! @yah:status(backlog)
//! @yah:parent(Q273)
//! @yah:next("F1: bring up warden raft on a single node (openraft, on-disk storage); storage backend choice (sled vs sqlite) resolved as F1 sub-spike")
//! @yah:next("F2: wireguard0 brought up + a single-node 'mesh' (one node, but the bus is live)")
//! @yah:next("F3: consume xlb-net::Endpoint as warden control-plane transport (xlb-net 0.1 published; Open Q2 from xlb-net doc finally gets a consumer)")
...
//! @yah:gotcha("R271 'Orbit architecture review for warden design' should resolve before F1 so its findings shape the storage-backend pick")
...
/// warden state file alongside the node's Tailscale mesh IP.
...
/// Concrete Raft type alias for the warden cluster.
...
/// The entire warden cluster state.  Serialised as a JSON snapshot;
...
pub struct RolloutRaftRecord {
    ...
    pub artifact: String,
    /// JSON-serialised `RolloutStatus` (avoids a cross-crate dep on warden's
...
/// Open (or create) a warden raft node.
///
/// `node_id`  — this machine's unique warden node ID (u64, assigned at provision time).
