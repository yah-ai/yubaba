//! `yah-warden` — per-machine infrastructure daemon.
//!
//! Modes:
//!
...
//!
//! @yah:ticket(R471-B7, "yah-warden container launches without `serve` subcommand → prints help, exits 2, restart-loops")
//! @yah:assignee(agent:claude)
//! @yah:at(2026-06-06T20:35:57Z)
//! @yah:status(review)
//! @yah:parent(R471)
//! @yah:severity(P1)
//! @yah:verify("Fresh container reaches `constable UDS listening` AND stays running >30s with `docker inspect` showing State.Status=running (not restarting).")
...
//! @yah:gotcha("Discovered as the proximate cause of the pond crash-loop that motivated R471. Container 37a1652... with image ghcr.io/yah-ai/yah-warden:latest: 14 restarts in 1m, exit code 2, policy=unless-stopped. Entry runs `yah-warden` with no subcommand. Either the Dockerfile CMD/ENTRYPOINT dropped `serve` or the clap root recently lost the implicit default. Check app/yah/yah-castle/Dockerfile + the wrapper script that exec's yah-warden.")
//! @yah:handoff("Root cause: `Cli::cmd: Cmd` was a required subcommand — a bare `yah-warden` invocation made clap print help and exit 2. That's what's crash-looping the pond container (Dockerfile CMD = pond-supervise.sh → `yah-warden ${YAH_WARDEN_ARGS:-}` with no subcommand).")
//! @yah:handoff("Fix is two-pronged: (1) main.rs flips `cmd` to `Option<Cmd>` and treats `None` as `Cmd::Serve` with every field at its declared default via a new `default_serve()` helper kept in lockstep with the variant; (2) pond-supervise.sh now invokes `yah-warden serve` explicitly so YAH_WARDEN_ARGS attaches unambiguously to the serve subcommand. Either fix alone would unstick pond; both ship for defense in depth.")
//! @yah:handoff("Verified locally: `/Users/user/ss/yah/target/debug/yah-warden` (no args) logs `INFO yah-warden serve` then `yah-warden listening` on 0.0.0.0:7443 (debug build is featureless so /workloads/deploy stays in stub mode — expected); `yah-warden --help` still lists serve/register-hostkey/raft. cargo test -p warden --lib → 96 passed.")
//! @yah:handoff("Image rebuild needed before the live pond container actually picks this up: `.yah/qed/build-yah-warden.toml` (arm64 local) rebuilds ghcr.io/yah-ai/yah-warden:latest into the local docker daemon; the GHA image-yah-warden job needs to retag for amd64 once main lands. Until that ships, the pond container at 37a1652... will keep crash-looping on the old image — that's a deploy step, not a code fix.")
...
//! @yah:next("Rebuild + push ghcr.io/yah-ai/yah-warden:latest so the pond container actually picks up the fix — this is the gating step for R471's outer verify (`Restarting (2) · N restarts in 1m` chip on the Services grid). Until then, B7's code is correct but the live fixture stays broken.")
//! @yah:verify("cargo build -p warden --bin yah-warden  # clean")
//! @yah:verify("cargo test -p warden --lib  # 96 passed")
//! @yah:verify("./target/debug/yah-warden  # logs 'yah-warden serve' + 'yah-warden listening', does NOT print help and exit 2")
//! @yah:verify("./target/debug/yah-warden --help  # still lists subcommands (root-level help unchanged)")
//! @yah:verify("Rebuild + reload the pond image (`yah qed run build-yah-warden` or equivalent), then `docker inspect <pond-warden>` shows State.Restarting=false and RestartCount stops climbing.")

use anyhow::{Context, Result};
use clap::{Parser, Subcommand};
use std::path::PathBuf;
use std::sync::Arc;
use warden::{identity, serve, ServerState, DEFAULT_BIND, DEFAULT_STATE_PATH};

/// Default raft state directory.
const DEFAULT_RAFT_DIR: &str = "/etc/yah-cloud/raft";
...
/// How long warden waits on a Constable UDS handshake at startup before
...
/// `ExecStart` timer; long enough to ride out the constable.service unit
...
struct Cli {
    /// Subcommand; defaults to `serve` so bare `yah-warden` boots the daemon.
    ///
    ...
    /// `ghcr.io/yah-ai/yah-warden:latest`. We treat `None` as Serve-with-defaults
...
enum Cmd {
    ...
    Serve {
        ...
        /// R406-T8: UDS path for the Constable sibling process. When set,
        /// warden dispatches workload list/state/drain through Constable
        /// over postcard-framed messages. If the connection fails at
        /// startup, warden falls back to the legacy in-process
        /// `ContainerRuntime` (with a warning) so existing single-node
        /// deploys keep working until T9 ships Constable's containerd
        /// backend.
        #[arg(long)]
        constable_socket: Option<PathBuf>,
        /// Phase 2: this node's raft node ID (u64, unique per warden instance).
    ...
    Raft {
        /// Warden daemon address to query (default: localhost).
...
async fn main() -> Result<()> {
    ...
    match cmd {
        Cmd::Serve {
            ...
            tracing::info!(channel = %channel, "yah-warden serve");
            ...
            // R406-T8: connect to Constable when --constable-socket is set.
            // Failure here is non-fatal so an operator can boot warden alone
            ...
            if let Some(node_id) = raft_node_id {
                ...
                let raft_node = warden::raft::open(node_id, raft_dir).await?;
                ...
                let _watcher = warden::leader::spawn(node_id, raft_node, Arc::clone(&shared_state));
                ...
                tracing::info!(
                    "warden running in single-node mode — \
...
async fn attach_runtime(mut state: ServerState, containerd_socket: &str) -> ServerState {
    ...
    {
        match constable_core::containerd::ContainerdRuntime::connect_at(containerd_socket).await {
            Ok(rt) => {
                tracing::info!(
    ...
    {
        tracing::warn!(
            "yah-warden built without the containerd-integration feature; \
...
/// Connect to Constable when `--constable-socket` is set; on failure, log a
/// warning and leave `constable_client = None` so warden falls back to the
...
async fn attach_constable_client(
    ...
    match constable_core::sibling::connect_with_timeout(
        socket.clone(),
    ...
    .await
    {
        Ok(client) => {
            tracing::info!(
                ...
                "constable client attached; workload list/state/drain dispatch \
                 through UDS"
            );
        ...
        Err(e) => {
            tracing::warn!(
                socket = %socket.display(),
                error = %e,
                "constable UDS connect failed; falling back to in-process \
...
fn init_tracing() {
    ...
    // `warden` = lib target; `yah_warden` = this binary crate (where the
    // serve-time runtime-attach + channel lines are emitted). Include both so
    // the stub-mode warning is visible by default.
    let filter = EnvFilter::try_from_default_env()
        .unwrap_or_else(|_| EnvFilter::new("warden=info,yah_warden=info,axum=info"));
