...
//!
//! Warden is the **only** writer of cheers's ownership table. On successful
...
//! into these writes — constable's `audit:write` scope is deliberately split
//! from warden's `ownership:write` so colocating constable on a warden host
...
//! bootstrap: warden holds an Ed25519 keypair allocated at install time
//! (`warden install` → `POST /admin/service-principals` → secret half written
//! once to warden's config dir, mode 0600). Cheers publishes the pubkey in its
//! JWKS so any verifier (cheers's own `/ownership` route, constable) can verify
//! warden-minted tokens through the same JWKS path it uses for cheers-minted
//! tokens — no separate fetch path.
//!
//! Tokens are PASETO v4.public, **minted locally** by warden — `sub: svc:<id>`,
...
//! minting means warden keeps writing ownership even during brief cheers API
...
//!   Returns `201 Created` + `OwnershipRow{id, ...}`. The `id` is what warden
...
/// Token TTL for warden's ownership-write calls. Bottom of the W159 §TTLs
...
pub enum CheersError {
    ...
    /// `warden install --rotate` to provision a fresh keypair.
...
pub struct CheersConfig {
    ...
    /// Service-principal id, e.g. `warden-prod-1`. Becomes the token's
    /// `sub` field (prefixed `svc:` on the wire per the canonical claim
    /// schema).
    pub principal_id: String,
    /// `kid` published in cheers's JWKS for warden's pubkey. Constable and
...
///
/// Construct once at warden startup via [`Self::new`] (reads the secret
...
impl CheersClient {
    ...
    /// Mint a `ownership:write`-scoped token, signed locally with warden's
    ...
    fn mint_ownership_write_token(&self, now_secs: i64) -> Result<String, CheersError> {
        ...
        let claims = serde_json::json!({
            ...
            "iat": now_secs,
            "jti": format!("warden-{now_secs}-{jti}"),
...
/// Subset of cheers's `OwnershipRow` shape — enough for warden to round-trip
...
/// fields warden doesn't care about land in `extra` as raw JSON.
...
mod tests {
    ...
    fn client_with(keypair: &AsymmetricKeyPair<V4>, issuer: &str) -> CheersClient {
        let cfg = CheersConfig {
            ...
            principal_id: "warden-test".to_string(),
            kid: "warden-test-1".to_string(),
        };
    ...
    fn mint_token_round_trips_and_carries_expected_claims() {
        ...
        assert_eq!(claims["sub"], "svc:warden-test");
        ...
        assert!(footer.contains(r#""kid":"warden-test-1""#));
    }
    ...
    async fn register_ownership_omits_on_behalf_of_when_none() {
        ...
        client
            .register_ownership("svc:warden-test", "machine", "m-1", "owns", None)
            .await
            .unwrap();
