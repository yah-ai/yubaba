//! Warden's MinIO supervision loop for the pond tier (R374-F3).
//!
...
//! `yah cloud mirror up`) shares them with the warden HTTP path. This
//! module wraps them with the warden-specific reconciler: probe loop +
...
//! + SigV4), but the reconciler holds a reference to warden's
...
// Re-export so users of `warden::pond::minio::*` see [`MinioSpec`] /
