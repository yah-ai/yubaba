...
//! `/pond/deploy` for each declared pond mirror at startup. Warden owns the
...
//! ```text
//!   POST /pond/deploy
//!   └─ warden::pond::minio::ensure_minio_running    (image-pull / run / probe / bucket-policy)
...
//! to drive the miniflare half — warden spawns it directly from the
...
//! @arch:see(.yah/docs/working/W180-pond-richer-topology.md)
//!
//! @yah:ticket(R455-F3, "pond_mesofact_dev local-driver module + PondDeployReq field + warden MesofactDevReconciler with HTTP probes")
...
//! @yah:next("Warden gets MesofactDevReconciler paralleling MinioReconciler/MiniflareReconciler: 5s loop, HTTP GET on liveness_path → restart; HTTP GET on readiness_path → mark not-ready (no restart)")
//! @yah:verify("cargo test -p warden --lib pond")
//! @yah:verify("Smoke: external HTTP kill of mesofact-dev → warden Degraded within probe interval → restart → Running")
//! @arch:see(.yah/docs/working/W180-pond-richer-topology.md)
...
///
/// `minio` carries the warden-owned MinIO slot spec (image, ports, data dir,
/// container name). Camp builds this from the workspace's
/// `providers.object_store` slot.
///
/// `miniflare` carries everything warden needs to spawn (and restart) the
...
/// warden can restart without re-reading any workspace state (R374-F4).
...
pub struct PondDeployReq {
    ...
    /// MinIO spec — warden brings this up before miniflare.
    pub minio: MinioSpec,
    /// Miniflare spec — warden spawns and supervises this after MinIO is ready.
    ...
    /// has any `mode:"ssr"` route. Warden brings this up between MinIO and
    ...
    /// When present, warden brings up almanac-serve + issue-tracker in a single
...
/// - `Running` means warden's reconcilers last saw both MinIO and miniflare healthy.
...
/// Warden's per-workload supervision over the MinIO container. Lives inside
...
impl PondRegistry {
    ...
    /// Insert a Running record with warden-supervised MinIO, miniflare, SSR
    ...
    /// dropping the camp daemon reaps every pond container + process warden
    /// is tracking.
...
/// warden instance). Returns 500 with a stage tag if MinIO or miniflare bring-up
/// fails.
pub(crate) async fn deploy(
    ...
    let ssr_running = if let Some(ref ssr_spec) = req.ssr_runtime {
        ...
        .await
        {
            Ok(running) => {
                ...
                // host port at spec-build time; warden owns the final wiring.
    ...
    // warden side using the MinIO container's bridge endpoint. Keeps camp
