...
use crate::config::{MachineConfig, LegacyServiceConfig};
use crate::mesh_service;

/// A rendered compose bundle ready to push to the warden's `POST /compose`.
#[derive(Debug, Clone)]
pub struct ComposeBundle {
    ...
    /// Shell commands the warden must execute after writing compose files —
...
pub fn generate_compose_bundle(
    ...
    let network = machine_network(machine);
    let compose_yaml = build_compose_yaml(machine, services, &network);
    let caddyfile = build_caddyfile(services, public_hostname);
    let firewall_cmds = collect_firewall_cmds(services);
    ComposeBundle { compose_yaml, caddyfile, firewall_cmds }
}
...
fn build_compose_yaml(
    ...
    for svc in services {
        append_service(&mut out, svc, network);
    }

    if has_public {
        append_caddy_service(&mut out, network);
    }
...
/// pair, mirroring the warden-7443 pattern established in `mirror.yml`.
...
fn append_caddy_service(out: &mut String, network: &str) {
    out.push_str("  caddy:\n");
...
mod tests {
    use super::*;
    use crate::config::{BucketSpec, MachineConfig, PortMapping, LegacyServiceConfig}; // PortMapping used in test constructors
    ...
    fn mesh_bound_service(name: &str, port: u16) -> LegacyServiceConfig {
        LegacyServiceConfig {
            ...
            version: "v1.0.0".into(),
            env: HashMap::new(),
            ports: vec![PortMapping { host: port, container: port }],
            mesh_only: true,
    ...
    fn mesh_only_service_has_no_caddy() {
        ...
        assert!(!bundle.compose_yaml.contains("caddy:"), "caddy should not appear");
    ...
    fn public_service_includes_caddy_and_caddyfile() {
        ...
        let bundle = generate_compose_bundle(&m, &svcs, None);
        assert!(bundle.compose_yaml.contains("caddy:"), "caddy missing from compose");
        assert!(bundle.compose_yaml.contains("caddy_data:"), "volume missing");
        assert!(bundle.caddyfile.is_some(), "Caddyfile expected for public service");
    }
    ...
    fn tier_tag_becomes_network_name() {
        ...
        let bundle = generate_compose_bundle(&m, &svcs, None);
        assert!(bundle.compose_yaml.contains("tier-t2:"), "network name mismatch");
        assert!(!bundle.compose_yaml.contains("yah-cloud:"), "fallback network present");
    }
    ...
    fn no_tier_tag_falls_back_to_yah_cloud_network() {
        ...
        let svcs: [LegacyServiceConfig; 0] = [];
        let bundle = generate_compose_bundle(&m, &svcs, None);
        assert!(bundle.compose_yaml.contains("yah-cloud:"), "fallback network missing");
    }
    ...
    fn image_includes_version() {
        ...
        let svcs = [service("asset-registry", true)];
        let bundle = generate_compose_bundle(&m, &svcs, None);
        assert!(bundle.compose_yaml.contains("ghcr.io/test/asset-registry:v1.0.0"));
    }
    ...
    fn caddyfile_uses_hostname_when_provided() {
        ...
        assert!(cf.contains("pdx.cloud.example.com {"), "hostname missing from Caddyfile");
        assert!(cf.contains("reverse_proxy api:8080"), "reverse_proxy missing");
    }
    ...
    fn mixed_services_only_routes_public_in_caddyfile() {
        ...
        assert!(cf.contains("public-svc"), "public service missing from Caddyfile");
        assert!(!cf.contains("mesh-svc"), "mesh-only service leaked into Caddyfile");
    }
    ...
    fn bind_interface_service_has_no_expose_block() {
        ...
        assert!(!block.contains("expose:"), "expose: must be omitted in host mode:\n{block}");
    }
    ...
    fn bind_interface_populates_firewall_cmds() {
        ...
        let svcs = [mesh_bound_service("postgres", 5432)];
        let bundle = generate_compose_bundle(&m, &svcs, None);
        assert_eq!(bundle.firewall_cmds.len(), 2, "expected 2 ufw rules: {:?}", bundle.firewall_cmds);
    ...
    fn no_bind_interface_produces_empty_firewall_cmds() {
        ...
        let bundle = generate_compose_bundle(&m, &svcs, None);
        assert!(bundle.firewall_cmds.is_empty(), "no bind_interface → no firewall cmds");
    }

    #[test]
    fn multiple_bound_services_accumulate_firewall_cmds() {
        let m = machine(&[]);
        let svcs = [mesh_bound_service("postgres", 5432), mesh_bound_service("nats", 4222)];
        let bundle = generate_compose_bundle(&m, &svcs, None);
        // 2 rules per service × 2 services = 4
        assert_eq!(bundle.firewall_cmds.len(), 4, "unexpected rules: {:?}", bundle.firewall_cmds);
    ...
    fn bind_interface_service_excluded_from_caddyfile() {
        ...
        let bundle = generate_compose_bundle(&m, &svcs, None);
        let cf = bundle.caddyfile.as_deref().unwrap();
        assert!(!cf.contains("postgres"), "bound service must not be in Caddyfile");
