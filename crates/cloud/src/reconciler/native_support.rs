...
/// unchanged.
///
/// R918-F5 — unix-only: its only caller is [`super::local_process`], which is
/// gated for the reap ladder it needs.
#[cfg(unix)]
...
///
/// R918-F5 — unix-only outside tests: its only non-test caller is
/// [`spawn_native_log_supervisor`], gated just above.
#[cfg(any(unix, test))]
