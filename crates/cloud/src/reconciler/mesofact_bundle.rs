...
//! @yah:parent(R870)
//! @yah:severity(high)
//! @yah:verify("From the noisetable camp: with .yah/services/noisetable-marketing/service.toml unchanged apart from removing its stopgap ordering note, one 'yah cloud apply --service noisetable-marketing --env cloud' leaves BOTH https://noisetable.com/ and https://noisetable.com/app/ serving 200 — the second is the assertion that matters, it has been 404 since the door went live.")
...
//! @yah:verify("The stopgap is gone: .yah/services/noisetable-marketing/service.toml contains no component-ordering note, and one apply leaves BOTH https://noisetable.com/ and https://noisetable.com/app/ at 200 regardless of the order the two components are declared in.")
...
//! @yah:gotcha("NOISETABLE IS LIVE ON A STOPGAP AND IT IS IN THE TREE, not just here: .yah/services/noisetable-marketing/service.toml orders the 'app' component BEFORE 'site' so the site wins the overwrite race, with a comment naming this ticket. https://noisetable.com/ and /account are 200; /app/ is 404. Closing this with config 1 means deleting that ordering note — the components go back to any order because order stops mattering.")
