...
/// to user principals (yubaba carries `ownership:write`; kamaji's own
...
/// Protected-resource metadata document, RFC 9728 §2 shape. Kamaji serves
...
pub struct ProtectedResourceMetadata {
    /// The protected resource's identifier — this kamaji's `aud` URI.
    pub resource: String,
    /// AS issuer URLs cheers's OAuth token-endpoint lives at.
    pub authorization_servers: Vec<String>,
    /// Scopes kamaji accepts on the wire — see [`SCOPE_VOCABULARY`].
...
impl ProtectedResourceMetadata {
    ...
    /// published scope set — useful for a kamaji instance that has been
...
mod tests {
    ...
    fn from_config_publishes_full_vocabulary() {
        let cfg = AuthConfig::new("https://cheers.example", "https://kamaji.example");
        let meta = ProtectedResourceMetadata::from_config(&cfg);
        assert_eq!(meta.resource, "https://kamaji.example");
        assert_eq!(
    ...
    fn service_only_scopes_advertised() {
        ...
        // accepts. Service principals (yubaba) carry `ownership:write`;
        // kamaji's own forwarder carries `audit:write`. Both are
