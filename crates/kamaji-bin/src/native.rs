//! Kamaji's native workload spawn surface (R406-T5).
//!
//! Pipeline for a native workload:
//!
//! 1. Yubaba → Kamaji `Deploy(WorkloadSpec)` over the UDS.
//! 2. Kamaji creates a per-workload cgroup via [`crate::CgroupV2`] (R406-T4).
//! 3. Kamaji calls [`spawn`] (this module) — `fork`, parent attaches the
...
//!   — musl-static binaries can't resolve nsswitch reliably, and Kamaji
//!   intentionally does not link nss plugins. `None` means "stay at Kamaji's
...
//!   `FromMesh` references are Yubaba's responsibility to resolve before the
//!   spec reaches Kamaji; unresolved variants are rejected.
...
pub struct SandboxPlan {
    ...
    /// uid/gid drop target. `None` keeps Kamaji's current uid.
...
///
/// Kamaji always installs a ruleset that defaults to denying writes
...
/// - `stdout` / `stderr` — read ends of the pipes Kamaji retains; the
        ...
        match &e.value {
            ...
            EnvValue::FromSecret { .. } => {
                return Err(SpawnError::InvalidSpec(format!(
                    "env {name:?} is a FromSecret ref; Yubaba must resolve secret refs before \
                     dispatching to Kamaji",
                    name = e.name
                )));
            }
            EnvValue::FromMesh { .. } => {
                return Err(SpawnError::InvalidSpec(format!(
                    "env {name:?} is a FromMesh ref; Yubaba must resolve mesh refs before \
                     dispatching to Kamaji",
                    name = e.name
                )));
            }
...
fn derive_landlock(volumes: &[VolumeMount]) -> LandlockPolicy {
    ...
    for v in volumes {
        ...
        // Named volumes need yubaba-managed mount setup; Tmpfs needs a mount
...
mod linux {
    ...
    pub fn spawn(plan: &SandboxPlan, cgroup: &CgroupHandle) -> Result<NativeChild, SpawnError> {
        ...
        // exec so the workload binary doesn't see Kamaji's internal fds.
    ...
    fn pre_exec_in_child(
        ...
        // Redirect stdout/stderr to the pipes Kamaji owns. dup2 clears
