...
//! kamaji's first-start fetch path actually reaches a live `/.well-known/`
...
use kamaji_bin::{AuthConfig, AuthVerifier, VerifyError};
...
async fn boots_and_verifies_against_mock() {
    ...
    // wire shape drifted from what kamaji's JwksDoc accepts.
...
async fn boot_can_serve_from_cache_after_mock_dies() {
    ...
    // Mock dies; kamaji restarts. Cache is fresh (just-written), so the
