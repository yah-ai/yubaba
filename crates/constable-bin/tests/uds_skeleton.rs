...
async fn read_one_reply(stream: &mut UnixStream) -> ConstableToWarden {
    ...
    loop {
        match decode_frame::<ConstableToWarden>(&buf) {
            Ok((msg, _)) => return msg,
            Err(constable_proto::Error::Truncated { .. }) => {
...
async fn end_to_end_hello_list_and_stop_skeleton() {
    ...
    let socket = dir.path().join("constable.sock");

    let (stop_tx, stop_rx) = oneshot::channel::<()>();
    let server_path = socket.clone();
    let server = tokio::spawn(async move {
        constable::serve_with_shutdown(&server_path, async move {
            let _ = stop_rx.await;
        })
        .await
    });
    ...
    // teardown is exercised in the warden↔constable integration test once
