//! Constable's containerd backend (R406-T9).
//!
//! ## Why this lives in Constable, not in Warden
//!
//! Per [W154](../../../../.yah/docs/working/W154-warden-dual-runtime.md)
//! §"Constable's native driver" and §"Impact on warden codebase":
//!
//! > Existing crates/yah/warden/: trimmed to mesh/raft/admission/federation.
//! > Drops direct knowledge of containerd internals; talks to Constable over
//! > UDS for all workload lifecycle.
//!
//! Containerd is one of Constable's two backends (the other is `native` —
//! see [`crate::native`]). Both consume the same enriched `WorkloadSpec`
//! after Constable applies the WorkloadSpec enforcement layer (capabilities,
//! secret mounts, MeshIdent-aware bindings). Warden owns admission and
...
//! the dev binary and pond's inner constable (which uses the host docker
//! socket via a separate backend) don't carry tonic + the containerd-client
//! stack.
//!
//! ## Shape
//!
//! - One `ContainerdBackend` per Constable instance, holding the tonic
...
//!   across Constable restarts so reconciliation can match).
...
use constable_proto::{WorkloadEntry, WorkloadId, WorkloadState};
use containerd_client::{
    services::v1::{
        container::Runtime as ContainerRuntime, containers_client::ContainersClient,
        images_client::ImagesClient, tasks_client::TasksClient,
        version_client::VersionClient, Container, CreateContainerRequest, CreateTaskRequest,
        DeleteContainerRequest, DeleteTaskRequest, GetContainerRequest, GetImageRequest,
...
/// constable.service systemd unit binds-mounts this path in the warden.slice.
...
/// reconciliation across Constable restarts can re-discover surviving
/// children.
...
/// write as the container's stdout/stderr; Constable opens them for read
...
pub enum BackendError {
    /// The provided workload spec failed validation Constable applies
...
/// Per-workload state Constable tracks for cancellation. R406-T10 stores
...
impl ContainerdBackend {
    ...
    /// Attach a log sink. The production binary passes the constable-wide
    ...
    /// Deploy a workload. The `id` is what Constable's registry keys on and
    ...
    pub async fn deploy(
        ...
        // Verify the image is in containerd's image store. Callers (warden
        // admission, R040-F11's bootstrap) are expected to have pre-pulled
        // it via `ctr images pull` or the MachineProvider bootstrap path.
        {
        ...
        info!(
            ...
            pid = pid,
            image = %image_ref,
            "constable: containerd workload deployed"
        );
        Ok(pid)
    }
    ...
    pub async fn teardown(&self, id: &WorkloadId) -> Result<(), BackendError> {
        ...
        // crashed Constable that left FIFOs behind needs them gone before
                ...
                if let Err(e) = tokio::fs::remove_file(&fifo).await {
                    if e.kind() != std::io::ErrorKind::NotFound {
                        warn!(
                            ...
                            fifo = %fifo.display(),
                            error = %e,
                            "constable: failed to unlink FIFO during teardown"
                        );
                    }
                }
            }
        ...
        info!(container_id = %container_id, "constable: containerd workload torn down");
        Ok(())
    }
...
/// running constable with the `containerd-integration` feature are an
    ...
    let join = tokio::spawn(async move {
            ...
            crate::journal::forward_reader(sink, workload, stream, recv).await
        {
            tracing::warn!(
                ...
                stream = stream.label(),
                error = %e,
                "constable: log forwarder ended on read error"
            );
        }
    });
...
/// Build labels Constable stamps on every container — these are how
...
/// recovers the workload id after a Constable restart.
...
/// `FromSecret` / `FromMesh` env values are warden's responsibility; if
/// they reach Constable it's a bug in warden's admission layer.
        ...
        match &env.value {
            ...
            EnvValue::FromSecret { secret, .. } => {
                return Err(BackendError::InvalidSpec(format!(
                    "env {} carries an unresolved FromSecret({secret}) — warden must resolve before Deploy",
                    env.name
                )));
            }
            EnvValue::FromMesh { ident, .. } => {
                return Err(BackendError::InvalidSpec(format!(
                    "env {} carries an unresolved FromMesh({}) — warden must resolve before Deploy",
                    env.name, ident.0
                )));
            }
        }
...
mod tests {
    ...
    fn validate_rejects_unresolved_from_secret() {
        ...
        assert!(msg.contains("warden must resolve"), "msg: {msg}");
    }
