//! cgroup v2 driver for Constable's native workload backend.
//!
...
/// Default slice root used in production. Constable's systemd unit sets
/// `Slice=warden.slice`, which causes systemd to materialize this directory.
pub const DEFAULT_SLICE_ROOT: &str = "/sys/fs/cgroup/warden.slice";

/// Controllers Constable's native driver requires enabled on the
...
impl CgroupV2 {
    ...
    /// `/sys/fs/cgroup/warden.slice`). The per-workload subtree is
    /// `<slice_root>/native`.
