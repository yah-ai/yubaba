...
//!
//! Constable owns stdout/stderr fan-in for both backends so workload logs
...
//!   Constable opens the FIFOs with `O_RDWR` (keeps a writer side open so
...
use constable_proto::WorkloadId;
use tokio::io::{AsyncBufReadExt, AsyncRead, BufReader};

/// Default path of journald's user-API socket on a Linux host with systemd.
/// The production cloud-tier `constable.service` runs as root and writes
/// here directly.
...
/// emitting one huge line would balloon the read buffer Constable holds.
...
/// Constable only ever sends Info (stdout) or Err (stderr) — workloads that
...
pub fn build_journal_payload(
    ...
    // SYSLOG_IDENTIFIER=constable\n — gives journalctl a clean default filter.
    payload.extend_from_slice(b"SYSLOG_IDENTIFIER=constable\n");
