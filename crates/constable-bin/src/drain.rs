//! Constable's drain enforcement state machine (R406-T7).
//!
//! ## Protocol shape
//!
//! Drain is a Warden-issued request carrying a [`DrainBudget`]
//! (`{ flush_ms, checkpoint_ms }`). Constable runs a single combined timer of
...
//! - **`ForceKilled`** — budget elapsed; Constable issued SIGKILL.
//!
//! Wire-level versioning rides the Warden↔Constable [`ProtocolVersion`]
...
//! Constable does not send a second signal at the `flush_ms` boundary. The
//! workload distinguishes phases by elapsed time in the SIGTERM-only floor;
//! once R406-T11 introduces the structured workload channel, Constable will
...
//!
//! Signals are delivered via `pidfd_send_signal(2)` — Constable holds the
...
use constable_proto::{DrainBudget, DrainOutcome, WorkloadId};
use thiserror::Error;

#[cfg(target_os = "linux")]
use std::os::fd::OwnedFd;

/// Wall-clock cap Constable waits for the kernel to publish the post-SIGKILL
...
/// `budget` is the (flush, checkpoint) window pair from Warden.
///
/// Returns:
///
...
pub fn classify_clean_exit(
    exit: constable_proto::ExitStatus,
    elapsed_ms: u32,
    budget: DrainBudget,
) -> DrainOutcome {
...
mod linux {
    ...
    use constable_proto::{DrainBudget, DrainOutcome, ExitStatus, WorkloadId};
    ...
    mod tests {
        ...
        async fn sigterm_ignoring_workload_is_force_killed() {
            // A shell child that traps SIGTERM and then sleeps. Constable's
...
mod tests {
    ...
    use super::*;
    use constable_proto::ExitStatus;
