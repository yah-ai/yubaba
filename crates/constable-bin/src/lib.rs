//! Constable — process supervisor sibling to Warden.
//!
//! This crate is a lib + bin. The library exposes the [`server`] surface so
//! integration tests can drive Constable in-process; the binary is a thin
...
//!
//! @yah:relay(R426, "JWT contract + verifier core (constable)")
...
//!
//! @yah:ticket(R426-F2, "Constable JWKS cache + signature verification (first-start, rotation, kid-miss refresh)")
...
//! @yah:next("Verify: `cargo test -p constable --locked --lib auth::` (22 pass), `cargo test -p constable --locked` (107 + 1 pass), `cargo check -p constable --locked --features containerd-integration` (clean modulo pre-existing pidfd warning).")
...
//! @yah:next("Non-blocking polish (defer until F3+ surfaces it): background refresh task is built but `spawn_refresh_task` is not wired into the constable main loop yet (no `Arc<AuthVerifier>` in `ServerCtx` until the hub-cheers-rpc adapter lands in R426-F6); stale-cache restart branch is logic-tested but not test-covered (would need a clock-injection refactor of `last_refresh` to drive without sleeping).")
//! @yah:handoff("F2 landed in `app/yah/constable/src/auth/` (six modules: `mod`, `config`, `claims`, `error`, `jwks`, `verifier`). Wire envelope decision: PASETO v4.public (matches cheers's session-token envelope; resolves W159 §Open questions → 'Wire envelope' in favour of consistency with `cheers-verify`). Service principals also publish their Ed25519 pubkey in the same JWKS so one cache + one verify path handles cheers's signing key + every service key uniformly (R426-F1 contract).")
...
//! @yah:handoff("Test coverage 22 in `auth::` (107 total `constable` lib tests + 1 integration green): claims round-trip (required-only + all-fields + principal-kind parse); config (jwks_url normalisation + defaults); jwks (parse OKP/Ed25519, skip foreign kty, reject broken entry + wrong length, atomic disk round-trip + parent-dir creation + load-NotFound); verifier (happy path, unknown-kid, signature mismatch, expired, bad iss, bad aud, missing footer = MissingKid, malformed footer = Malformed, boot-no-cache-no-AS = BootFetchFatal, kid-miss rate-limit gate behaviour).")
//! @yah:verify("cargo test -p constable --locked --lib auth::")
//! @yah:verify("cargo test -p constable --locked")
//! @yah:verify("cargo check -p constable --locked --features containerd-integration")
