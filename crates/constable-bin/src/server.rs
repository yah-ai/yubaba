//! @yah:ticket(R426-F3, "Constable scope + ownership-list check; canonical 401/403 + WWW-Authenticate bodies")
...
//! @yah:next("Sign-off: review `constable::auth::{policy,deny}` shape + run `cargo test -p constable --lib auth` (expect 39 auth-module passes, 127 total constable lib passes). Confirm the 401/403 examples in W159 §Failure responses match what `Deny::www_authenticate` + `Deny::json_body` emit byte-for-byte.")
//! @yah:verify("cargo test -p constable --lib auth")
//!
//! @yah:ticket(R426-F4, "Constable /.well-known/oauth-protected-resource endpoint")
//! @yah:assignee(agent:claude)
...
//! @yah:handoff("6 new unit tests green (full-vocab publish, trailing-slash trim, custom-scope override, JSON round-trip, RFC 9728 field-name presence, service-only scope advertisement). Total constable lib: 133 tests.")
//! @yah:next("Sign-off: review `constable::auth::metadata` shape + run `cargo test -p constable --lib auth::metadata` (expect 6 passes). Confirm `SCOPE_VOCABULARY` matches the W159 §Scope vocabulary list including the service-only scopes.")
//! @yah:verify("cargo test -p constable --lib auth::metadata")
...
/// Per-workload state Constable needs to drive a structured drain (R406-T7).
///
...
///   per workload Constable is supervising).
...
///   under R406-T8 when Constable starts owning workload lifecycle end-to-end)
...
/// Per-Constable runtime context handed to [`handle_message`] (R406-T9).
///
...
pub struct ServerCtx {
    ...
    /// `containerd-integration` feature build, or when constable is started
...
impl ServerCtx {
    ...
    /// pond-tier constable instances. The default log sink is a
...
impl Registry {
    ...
    /// owns the invariant that ids are unique per Constable lifetime.
...
/// [`ServerCtx`] — needed by `app/yah/constable/src/main.rs` so the
    ...
    F: Future<Output = ()>,
{
    ...
    info!(path = %socket.display(), "constable UDS listening");
...
pub async fn handle_message(
    ...
    match msg {
        ...
        WardenToConstable::Drain {
            ...
            // way Warden gets DrainAck { accepted=false, reason="unknown" }.
            ...
            // R406-T8 gives Constable a back-channel to Warden.
            let outcome =
        ...
        WardenToConstable::Probe { request_id, id } => {
            ...
            // Warden's already decided to drop — harmless.
        ...
        // The Warden→Constable enum is #[non_exhaustive]; reject any variant
...
/// (MesofactStatic, Almanac) are warden's responsibility (mesofact-static
/// reconciler / almanac runner) and surface as `InvalidSpec` if they reach
/// Constable.
#[allow(unused_variables)]
async fn deploy_workload(
    ctx: &Arc<ServerCtx>,
    request_id: constable_proto::RequestId,
    id: WorkloadId,
    ...
    match spec {
        workload_spec::Workload::Container(spec) => {
                ...
                let Some(backend) = ctx.containerd.clone() else {
                    return ConstableToWarden::Error {
                        ...
                        message: "no containerd backend configured — \
                                  rebuild constable with --features containerd-integration \
            ...
            #[cfg(not(feature = "containerd-integration"))]
            {
                ...
                ConstableToWarden::Error {
                    request_id: Some(request_id),
                    code: ErrorCode::BackendRefused,
                    message: "constable built without containerd-integration feature; \
        ...
        | workload_spec::Workload::StaticAsset(_) => ConstableToWarden::Error {
            request_id: Some(request_id),
            code: ErrorCode::InvalidSpec,
            message: "constable only dispatches Workload::Container; \
                      mesofact-static, almanac, and static-asset live in warden's reconcilers"
                .to_string(),
        },
    }
}
...
/// kill+delete; without a backend (or for a workload Constable doesn't know
...
async fn stop_workload(
    ...
    ConstableToWarden::Ack {
        request_id,
        kind: constable_proto::AckKind::Stop,
    }
}
...
fn drain_outcome_to_ack(
    ...
    match outcome {
        ...
        Ok(DrainOutcome::Unsupported) => (
            false,
            Some("drain not supported on this Constable build (non-Linux)".to_string()),
        ),
        // `DrainOutcome` is `#[non_exhaustive]` — future variants land in
        // constable-proto without a wire-version bump. Surface unknown
        // outcomes as not-accepted so a forward-version Constable replying
        // to an older Warden doesn't silently misreport success.
        Ok(other) => (
            false,
...
mod tests {
    use super::*;
    use constable_proto::{AckKind, DrainBudget, ExitStatus, RequestId, WorkloadId};
    ...
    /// MesofactStatic / Almanac workloads are not constable's concern — they
    /// belong to warden's reconcilers. Constable rejects them with InvalidSpec
    /// so warden surfaces the misroute clearly instead of silently dropping.
    #[tokio::test]
    async fn deploy_mesofact_static_is_rejected_as_invalid_spec() {
        ...
        match reply {
            ConstableToWarden::Error {
                ...
                assert!(
                    message.contains("mesofact-static") || message.contains("warden"),
                    "got: {message}"
                );
            }
    ...
    /// declared" convention. Probe-target absence ↔ Ready makes Warden's
    ...
    async fn probe_unregistered_workload_returns_ready() {
        ...
        match reply {
            ConstableToWarden::ProbeResult {
                ...
                assert!(
                    matches!(status, constable_proto::ProbeStatus::Ready),
