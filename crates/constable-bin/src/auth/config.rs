//! Auth-surface configuration. Defaults track W159 §Constable startup and
//! JWKS lifecycle exactly:
//!
//! - JWKS cache at `${XDG_CACHE_HOME:-~/.cache}/constable/jwks.json`
...
pub struct AuthConfig {
    ...
    /// This constable's resource URI — tokens must carry it in `aud`.
...
impl AuthConfig {
    ...
    /// `${XDG_CACHE_HOME:-${HOME}/.cache}/constable/jwks.json`.
    ...
    /// `/var/lib/constable/jwks.json`; tests use a `tempfile::TempDir`).
    ...
    /// Absolute URL of *this* constable's `/.well-known/oauth-protected-resource`
...
mod tests {
    ...
    fn jwks_url_handles_trailing_slash() {
        let c = AuthConfig::new("https://cheers.example/", "https://constable.example");
    ...
    fn resource_metadata_url_handles_trailing_slash() {
        let c = AuthConfig::new("https://i", "https://constable.example/");
        assert_eq!(
            c.resource_metadata_url(),
            "https://constable.example/.well-known/oauth-protected-resource"
        );
    }
