//! Canonical 401 / 403 wire shapes for the constable auth surface.
//!
...
//! WWW-Authenticate: Bearer realm="constable",
//!   error="invalid_token",
//!   error_description="signature verification failed",
//!   resource_metadata="https://constable.example/.well-known/oauth-protected-resource"
...
pub const DEFAULT_REALM: &str = "constable";
...
impl Deny {
    ...
    /// 5. `resource_metadata`
    ///
    /// `resource_metadata_url` is the absolute URL of *this* constable's
...
mod tests {
    ...
    #[test]
    fn realm_override() {
        let deny = Deny::invalid_token("token expired").with_realm("warden");
        let header = deny.www_authenticate(METADATA_URL);
        assert!(header.starts_with("Bearer realm=\"warden\","));
    }
