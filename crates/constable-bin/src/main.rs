...
const DEFAULT_SOCKET: &str = "/run/constable/constable.sock";
const ABOUT: &str = "constable — Warden's sibling process supervisor.\n\nReads workload-control messages from Warden over a unix domain socket and dispatches them to the containerd backend (R406-T9) or the native fork+exec path (R406-T5/T6). Run with --containerd-socket to enable container workloads.";
...
fn print_help() {
    println!("{ABOUT}");
    println!();
    println!("Usage: constable [--socket PATH] [--containerd-socket PATH]");
    println!();
    println!("Options:");
    println!(
...
fn run() -> Result<()> {
    ...
    let args = match parse_args() {
        ...
        Err(ParseError::VersionRequested) => {
            println!("constable {}", env!("CARGO_PKG_VERSION"));
            return Ok(());
        }
...
/// Assemble the [`constable::ServerCtx`]. Attaches a containerd backend when
...
/// without journald reachable, [`constable::JournalSender::connect`] falls
/// back to tracing — see `crate::journal` for the fallback path.
async fn build_ctx(
    containerd_socket: Option<PathBuf>,
) -> Result<Arc<constable::ServerCtx>> {
    let log_sink: std::sync::Arc<dyn constable::LogSink> =
        std::sync::Arc::new(constable::JournalSender::connect());
    #[cfg(feature = "containerd-integration")]
    {
        let ctx = constable::ServerCtx::new().with_log_sink(log_sink.clone());
        let ctx = if let Some(sock) = containerd_socket {
            let backend = constable::containerd::ContainerdBackend::connect_at(&sock)
                .await
                .map_err(|e| {
                    anyhow::anyhow!(
    ...
    #[cfg(not(feature = "containerd-integration"))]
    {
        if containerd_socket.is_some() {
            anyhow::bail!(
                "--containerd-socket requires the constable binary be built with \
                 --features containerd-integration"
            );
        }
        Ok(Arc::new(
            constable::ServerCtx::new().with_log_sink(log_sink),
        ))
    }
}
