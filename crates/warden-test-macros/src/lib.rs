//! `warden-test-macros` — `#[test_with_provider]` proc macro.
//!
//! ## Usage
//!
//! ```rust,ignore
...
//!     R: warden::runtime::ContainerRuntime,
//! {
...
/// paths (`::cloud::...`, `::warden::...`). The caller's Cargo.toml must have
/// `cloud` (with `local-docker` feature) and `warden` (with
...
fn emit_local_variant(fn_name: &Ident, inner_name: &Ident) -> TokenStream2 {
    ...
    quote! {
        ...
        async fn #variant_name() {
            ...
            let rt = match ::constable_core::containerd::ContainerdRuntime::connect().await {
                Ok(rt) => rt,
                Err(e) => {
                    eprintln!(
...
fn emit_smoke_variant(fn_name: &Ident, inner_name: &Ident) -> TokenStream2 {
    ...
    quote! {
        ...
        async fn #variant_name() {
            ...
            let rt = warden::runtime::DummyRuntime;
            #inner_name(p, rt).await;
        }
    }
}
