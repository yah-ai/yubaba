...
//!
//! The caller embeds Constable in its own process tree and holds an
//! `Arc<dyn Constable>`. This module owns the builder side: probe backends
...
//! `Arc<dyn Constable>` is one concrete backend. Per-WorkloadSpec routing
//! across multiple backends (the W199 §"Constable picks the backend"
//! vision) is downstream work and is intentionally not modeled here.

use std::sync::Arc;

use crate::probe::BackendAvailability;
use crate::{Backend, BackendUnavailable, Constable};

/// Pick an inlined Constable backend from a caller-supplied preference list.
///
...
/// let constable = Inlined::pick(
///     &availability,
///     &[Backend::Docker, Backend::Containerd],
///     |backend| match backend {
///         Backend::Docker => Ok(Arc::new(docker::DockerRuntime::new()) as Arc<dyn Constable>),
...
impl Inlined {
    ...
    pub fn pick<F>(
        ...
        prefer: &[Backend],
        factory: F,
    ) -> Result<Arc<dyn Constable>, BackendUnavailable>
    where
        F: FnOnce(Backend) -> Arc<dyn Constable>,
    {
...
mod tests {
    ...
    #[async_trait]
    impl Constable for StubBackend {
    ...
    fn pick_first_available_in_preference_order() {
        ...
        let result = Inlined::pick(&avail, &[Backend::Docker, Backend::Containerd], |b| {
            Arc::new(StubBackend(b)) as Arc<dyn Constable>
        })
        .unwrap();
