...
//!
//! Exercises the full yubaba ↔ cheers contract that R427-F1 wired:
//!
//! 1. POST /workloads/deploy with `requesting_camp_id` + `on_behalf_of_user`
//!    → yubaba provisions the workload → calls `POST /ownership` on the
//!    cheers issuer URL with a `svc:yubaba-*` PASETO v4.public Bearer token
//!    and the canonical claim block → stores the returned row id keyed by
//!    workload ident.
//! 2. POST /workloads/{ident}/destroy → yubaba tears down the workload →
...
//! ## Scope
//!
//! This file proves the **yubaba ↔ cheers** half of the W159 §"Ownership
//! writes" flow. The full T2 narrative ("agent → kamaji → yubaba →
//! cheers → camp refresh → 2nd deploy") needs two more pieces that aren't
//! in this relay:
//!
//! - the **authed transport** in front of yubaba (R426-F2's
//!   `hub-cheers-rpc` adapter) for the agent → kamaji hop; today the
//!   tests POST straight to yubaba's HTTP surface, which mirrors what the
...
/// key yubaba minted them with — the install-flow `--rotate` shape end to
/// end.
...
) -> (tempfile::TempDir, Arc<yubaba::ServerState>) {
    let tmp = tempfile::TempDir::new().unwrap();
    let cheers = CheersClient::new(
        CheersConfig {
            issuer_url: cheers_url.to_string(),
            principal_id: "yubaba-smoke".to_string(),
            kid: "yubaba-smoke-1".to_string(),
        },
        kp.secret.as_bytes(),
    )
    .unwrap();
...
async fn deploy_registers_then_destroy_revokes() {
    ...
    assert_eq!(claims["sub"], "svc:yubaba-smoke");
...
async fn second_deploy_writes_a_distinct_row() {
    ...
    // Distinct PASETO `jti`s — yubaba's monotonic counter is the replay
    ...
    // And the persisted row ids on yubaba's side are also distinct (they're
...
async fn register_failure_does_not_tear_down_workload() {
    ...
    *mock.fail_post.lock().await =
    ...
    // Mock saw the POST attempt but the row wasn't persisted on yubaba's
