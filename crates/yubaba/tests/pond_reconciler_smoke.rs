...
//! yubaba's pond deploy handler, kill each in turn, assert reconcilers flip
...
//! After R374-F4, yubaba owns both slots — MinIO (container) and miniflare
...
//! @yah:next("After the fixture compiles, run `cargo test -p yubaba --test pond_reconciler_smoke` to confirm reconciler behavior unchanged.")
//! @yah:verify("cargo test -p yubaba --test pond_reconciler_smoke passes")
//! @yah:handoff("Added mesofact_dev: None to PondDeployReq fixture in pond_reconciler_smoke.rs:261. cargo test -p yubaba --test pond_reconciler_smoke passes.")
...
async fn warden_reconciler_restarts_minio_and_miniflare() {
    if std::env::var("YAH_LOCAL_SIM_E2E").as_deref() != Ok("1") {
        eprintln!(
            "[reconciler-smoke] SKIP: set YAH_LOCAL_SIM_E2E=1 to run the yubaba \
    ...
    let server_state = yubaba::ServerState::load(state_path.clone()).expect("ServerState::load");
    let server_state = Arc::new(server_state.with_pond_local_runtime(runtime.clone()));

    let server_handle = tokio::spawn({
        let s = server_state.clone();
        async move {
            let _ = yubaba::serve_on_listener(listener, s).await;
        }
    });
    ...
    .unwrap_or_else(|last| {
        panic!(
            "yubaba did not report Running within {:?}; last seen: {:?}",
            DEPLOY_BUDGET, last
        )
    });
    ...
    .unwrap_or_else(|last| {
        panic!(
            "yubaba did not restart MinIO within {:?} after kill; last seen: {:?}",
            RECOVER_BUDGET, last
        )
    });
    eprintln!(
