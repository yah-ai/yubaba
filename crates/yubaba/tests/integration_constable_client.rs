...
//!
//! When `R406-T9` ships Kamaji's containerd backend, this test gains a
//! `deploy` round-trip (currently the Kamaji handler short-circuits to
//! `Error { Internal, "backend driver not implemented" }`).
//!
//! @yah:ticket(R427-T2, "E2E smoke: agent → kamaji → yubaba → cheers ownership write → camp refresh → 2nd deploy")
...
//! @yah:handoff("E2E ownership-write smoke landed at crates/yah/yubaba/tests/integration_ownership_smoke.rs. Spawns an in-process axum cheers mock (POST /ownership returning 201 + monotonic 01HROW#### ids; DELETE /ownership/{id} returning 204; failure injection toggle for the 5xx-during-register case) and a real yubaba ServerState wired with FakeRuntime + a real CheersClient pointed at the mock. Each test verifies the Bearer token cheers received by re-running PASETO v4.public verification against the same Ed25519 keypair yubaba minted with — same JWKS-shape path cheers's actual ownership router uses, just shrunk to a single test.")
//! @yah:handoff("FOUR CASES COVERED. (1) deploy_registers_then_destroy_revokes: full happy path — POST body matches the canonical CreateOwnershipBody shape (principal_id/resource_kind/resource_id/relationship/on_behalf_of), Bearer token sub=svc:yubaba-smoke + scope=[ownership:write] + iss==aud==issuer_url + TTL inside the W159 5–15 min band, returned row_id round-trips into yubaba's ownership_rows map, destroy DELETE hits the same id with a freshly-minted token, map entry consumed. (2) second_deploy_writes_a_distinct_row: two sequential deploys against the same camp — distinct PASETO jti's (yubaba's monotonic counter is the same-second replay backstop), distinct resource_ids on the wire, distinct row_ids in the map. This is the 'camp refresh → 2nd deploy' half of the T2 narrative reduced to what's testable today. (3) deploy_without_camp_id_skips_register: no requesting_camp_id ⇒ no cheers call at all (dev-tier shape). (4) register_failure_does_not_tear_down_workload: cheers returns 500 on POST — deploy still returns 200 deployed, no row stored on yubaba's side, workload stays up (W159 'auth-table blip is worse than audit gap' invariant).")
//! @yah:handoff("DEFERRED FROM THE FULL T2 NARRATIVE (documented in the test file header). Two pieces of 'agent → kamaji → yubaba → cheers → camp refresh → 2nd deploy' need design work outside this relay: (a) the authed transport in front of yubaba (R426-F2 hub-cheers-rpc adapter) for the agent→kamaji hop — today this test POSTs straight to yubaba's HTTP surface, which is exactly what the adapter will do once it lands; (b) cheers's token-mint endpoint + camp refresh path — today the tests stub camp:/user: ids on the deploy body; R428 replaces them with derivation from verified MCP claims. Neither gap is a behavioral hole in R427-F1 — they're upstream wiring that mints the inputs to what this relay landed.")
//! @yah:verify("cargo test -p yubaba --features testing --test integration_ownership_smoke  # 4 passed")
//! @yah:verify("cargo test -p yubaba --features testing  # 121 lib + 4 ownership_smoke + integration suites green")
