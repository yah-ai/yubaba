//! `yubaba::runtime` — historical home of the `ContainerRuntime` trait.
//!
//! As of R484-T5 the trait + concrete backends (containerd / docker / fake)
//! live in the `kamaji-core` crate and yubaba's call sites import them
...
//!
//! @yah:ticket(R091-F1, "ContainerRuntime trait + runtime::containerd impl: replace yubaba's shell-out path; JSON tracing + correlation IDs")
//! @yah:at(2026-06-08T21:47:23Z)
//! @yah:status(review)
//! @yah:see(.yah/docs/architecture/A053-yah-yubaba-integration-testing.md)
//!
//! @yah:relay(R276, "Tier 3 — Real yubaba workload runtime (single-node deploy)")
...
//! @yah:verify("cargo test -p yubaba --lib  # 62 passed, including health_returns_ok asserting mode='single-node'")
//! @yah:verify("curl http://<yubaba>:7443/health | jq .mode  # single-node without --raft-node-id, clustered with it")
//! @yah:verify("grep 'containerd-integration' .github/workflows/release.yml  # yubaba-release job now includes the feature flag")
...
//! @yah:gotcha("Unblocks Q156 mirror-sage (R157/R158/R159 queued in handoff)")
//! @yah:gotcha("Promote to quest if F2 alone grows past three sub-tickets")
//! @yah:next("F2 (E2E — human): All code is committed in the working tree (uncommitted). Commit yubaba/src/ + .github/workflows/release.yml, cut a new tag to trigger release.yml, get the yubaba-<tag>-x86_64-unknown-linux-musl.tar.gz URL from the GitHub release assets, create .yah/cloud/workloads/yubaba-probe.toml with a minimal workload (nginx:alpine, tier=public), then: yah cloud machine provision yah-cloud-1 --yubaba-url <URL> && yah cloud workload deploy yubaba-probe --machine yah-cloud-1. Verify yubaba logs show 'containerd runtime attached' (not stub mode) and /workloads shows the container Running.")
//! @yah:handoff("F1/F3/F4 DONE, release.yml fix DONE — all code work complete. 62 yubaba lib tests pass; cargo check clean in both feature modes. All changes UNCOMMITTED in the working tree (yubaba/src/, .github/workflows/release.yml, leader.rs _node_id fix). F2 E2E is the only remaining step and needs human action: commit + tag + provision + deploy. workloads load from .yah/cloud/workloads/ (gitignored; create locally). yah-cloud-1 machine config exists in .yah/infra/machines/ but the Hetzner server is not provisioned. Rollout code in yubaba/src/rollout/ is R278 work and should be committed together.")
//! @yah:verify("git diff --stat HEAD  # shows yubaba/src/, release.yml, leader.rs all modified")
//! @yah:verify("cargo check -p yubaba && cargo check -p yubaba --features containerd-integration  # both clean")
//! @yah:verify("cargo test -p yubaba --lib  # 62 passed")
//! @yah:verify("grep 'containerd-integration' .github/workflows/release.yml  # yubaba-release job has the flag")
//! @yah:verify("curl http://<yubaba>:7443/health | jq .mode  # single-node (no --raft-node-id), clustered (with it)")
//!
//! @yah:ticket(R256-F10, "KEYSTONE: converge cloud::local_runtime + yubaba::runtime::containerd into one Runtime trait")
//! @yah:assignee(agent:claude)
//! @yah:at(2026-05-25T21:46:49Z)
//! @yah:status(review)
//! @yah:parent(R256)
//! @yah:next("one Runtime trait — deploy + supervise a WorkloadSpec — with two backends: orbstack/docker socket AND containerd. camp embeds it for dev+sim (orbstack); yubaba for cloud+ha (containerd)")
...
//! @yah:next("today there are two impls: cloud::local_runtime (docker CLI, drives Caddy+MinIO) and yubaba::runtime::containerd (gRPC). Unify behind the trait; sim keeps orbstack as the backend (upholds the dev-yah-static-demo locked decision)")
//! @yah:gotcha("main design call: where the shared trait lives — a new shared crate both cloud + yubaba depend on, or yubaba::runtime as canonical with cloud depending on it. Pick before splitting impls")
//! @arch:see(.yah/docs/architecture/A024-vocabulary.md)
//! @yah:next("REALIZED by R374 — the keystone shipped end-to-end in pond. R374-F3 extracted local-driver (docker-CLI driver) as the sim-side WorkloadRuntime backend; R374-F3/F4 wired yubaba's MinioReconciler + MiniflareReconciler against it. Cloud and yubaba now share local-driver as their sim/pond container lifecycle path — the camp-on-bespoke-docker shortcut is gone. Containerd-side WorkloadRuntime impl still lands under R276 (yubaba's ContainerRuntime), as F10's original handoff foretold. See W142 §Crate layout + .yah/docs/working/W142-pond.md.")
//! @yah:handoff("WorkloadRuntime trait added to workload-spec (async_trait + anyhow added as deps). Trait has 4 methods: deploy_workload/teardown_workload/is_running/runtime_health. ImageRef::docker_ref() helper added. LocalDockerRuntime added to cloud/src/local_runtime.rs with workload_spec_to_crs translator (literal env only, bind mounts only, mesh/secrets skipped with warn). LocalDockerRuntime exported from cloud::lib. 6 unit tests covering translation, env filtering, port mapping, command override, and ImageRef::docker_ref. 171 cloud tests pass. Design decision: trait lives in workload-spec (already depended on by both cloud and yubaba, no new crate). Yubaba's ContainerRuntime (mesh-aware gRPC impl) implements WorkloadRuntime in R276 Tier-3; the sim tier (camp) uses LocalDockerRuntime now. Reconcilers calling LocalRuntime directly are unchanged; LocalDockerRuntime is available for callers that want to deploy arbitrary WorkloadSpec containers at sim tier.")
...
//! @yah:status(review)
//! @yah:parent(R471)
//! @yah:verify("WorkloadStatus enum has Restarting variant; serde tag round-trips through the yubaba HTTP API.")
...
//! @yah:gotcha("S1 verdict: containerd has NO native Restarting state and NO restart-count. The containerd Status enum is {Unknown, Created, Running, Stopped, Paused, Pausing}. Restart loop is yubaba's job per RestartPolicy in workload-spec.rs:1186.")
...
//! @yah:handoff("Tests: 7 new (6 in runtime::containerd::tests, 1 in runtime::fake::tests) — ledger record/clear semantics, apply_ledger upgrade matrix, serde round-trip through JSON ({type:\"restarting\",...}), fake mark_restarting fixture. cargo test -p yubaba --features containerd-integration,testing --lib → 119 passed (was 109).")
//! @yah:next("Sign-off check: read crates/yah/yubaba/src/runtime/mod.rs:160-205 (enum + is_terminal) and crates/yah/yubaba/src/runtime/containerd.rs:30-205 (RestartLedger + apply_ledger). If shape looks right, archive R471-T2 — then R471-F3 (docker impl) and R471-T5 (Services grid wiring) can both pick up the enum.")
//! @yah:next("Follow-on (NOT this ticket): the supervisor side that actually calls ledger.record_exit / ledger.mark_running on each restart cycle lives wherever RestartPolicy is applied in yubaba's deploy/monitor loop — needs its own ticket if it isn't already covered by an existing sub-ticket of R276 or B7.")
//! @yah:verify("cargo check -p yubaba && cargo check -p yubaba --features containerd-integration  # both clean")
//! @yah:verify("cargo test -p yubaba --features containerd-integration,testing --lib  # 119 passed (7 new)")
//! @yah:verify("grep -n 'Restarting' crates/yah/yubaba/src/runtime/mod.rs  # WorkloadStatus::Restarting present")
//!
...
//! @arch:see(.yah/docs/architecture/A043-yah-on-machine-daemons.md)
//!
//! @yah:ticket(R484-T2, "Move runtime/{native,containerd,docker} from yubaba into crates/yah/kamaji")
//! @yah:assignee(agent:claude)
...
//! @yah:handoff("Moved yubaba/src/runtime/{containerd,docker,fake}.rs into crates/yah/kamaji/src/. ~2.4k LoC relocated verbatim, imports rewired (`use crate::mesh::MeshAssignment; use super::{...}` → `use crate::{Backend, Kamaji, MeshAssignment, ...}`). Each impl block was renamed `impl ContainerRuntime for X` → `impl Kamaji for X` and got a `fn backend() -> Backend` returning Containerd / Docker / Containerd (FakeRuntime stands in for the containerd backend in unit tests). `MeshAssignment::stub(ip)` kept as an alias of `MeshAssignment::inlined(ip)` so existing yubaba call sites compile untouched.")
//! @yah:handoff("yubaba::mesh::MeshAssignment + WireguardPeer are now re-exports of constable_core types (yubaba/src/mesh/mod.rs is 16 lines, no struct defs left). yubaba/src/runtime/mod.rs is a re-export shim: `pub use constable_core::{Kamaji as ContainerRuntime, DeployResult, LogEvent, LogOpts, LogStream, LogStreamKind, RuntimeHealth, WorkloadState, WorkloadStatus};` plus feature-gated module re-exports (`pub use constable_core::containerd`, `::docker`, `::fake`). DummyRuntime stays in yubaba::runtime since it's smoke-tier-specific; updated to `impl Kamaji` with `fn backend()` returning Backend::Containerd. R484-T5 deletes this shim and migrates callers onto `constable_core::*` directly.")
//! @yah:handoff("Cargo wiring: kamaji-core got `[features] containerd-integration/docker-integration/testing` matching yubaba's; optional `containerd-client = 0.9` + `prost-types = 0.14` moved into kamaji-core's deps gated on containerd-integration. kamaji-core's regular deps: anyhow, async-trait, futures-core, serde, serde_json, thiserror, tokio (rt+macros+net+io-util+sync+fs+time+process), tokio-stream, tracing, workload-spec. yubaba's three matching features now forward via `kamaji-core/containerd-integration` etc.; yubaba keeps its own copies of containerd-client + prost-types because main.rs / cloud-init still references containerd types directly (cleanable in T5). yubaba picks up `kamaji-core = { path = \"../kamaji\" }` as a regular dep.")
//! @yah:handoff("@yah:ticket annotation handling: original R091-F1 lives in yubaba/src/runtime/mod.rs (not duplicated to the moved containerd.rs — dropped from the copy with a comment line pointing back). R091-F2 (the runtime::fake ticket) MOVED with fake.rs into kamaji/src/fake.rs since mod.rs didn't have a copy — the annotation source-of-truth follows the file. R484-T2's own annotation stays in yubaba/src/runtime/mod.rs since the board IDs the relay's source from there (line 122).")
//! @yah:handoff("Verified: `cargo check -p kamaji-core --all-features` clean, `cargo test -p kamaji-core --all-features` 42 passed (3 in lib::tests + 12 in fake + 12 in docker + 12 in containerd + 3 verifying serde tags / inlined sentinel / terminal matrix). `cargo check -p yubaba` clean, `cargo check -p yubaba --features containerd-integration,testing,docker-integration` clean, `cargo test -p yubaba --lib --features containerd-integration,testing,docker-integration` 108 passed (matches previous baseline; runtime tests now run in kamaji-core, not yubaba — expected). `cargo check --workspace --all-targets` clean, `cargo check -p yubaba --tests --features containerd-integration,testing,docker-integration` clean (integration tests compile via the shim).")
//! @yah:next("Sign-off check: skim crates/yah/kamaji/src/lib.rs (trait + types), crates/yah/kamaji/src/{containerd,docker,fake}.rs (impl blocks now `impl Kamaji for X` with `backend()`), and the yubaba shim crates/yah/yubaba/src/runtime/mod.rs. Then archive R484-T2.")
//! @yah:next("T3 picks up: Backend probe-at-init wiring (R484-T3). The Backend enum + BackendUnavailable error landed in T1; T3 adds the connect()-test probes for docker/containerd sockets at Kamaji init and uses BackendUnavailable when callers ask for an absent backend. Probe semantics already described in W199 §Backend availability — lift the install-hint strings from there.")
//! @yah:verify("cargo check -p kamaji-core --all-features  # clean")
//! @yah:verify("cargo test -p kamaji-core --all-features  # 42 passed")
//! @yah:verify("cargo check -p yubaba --features containerd-integration,testing,docker-integration  # clean")
//! @yah:verify("cargo test -p yubaba --lib --features containerd-integration,testing  # 108 passed")
//! @yah:verify("cargo check --workspace --all-targets  # clean (warnings only, no errors)")
//! @yah:verify("grep -c 'pub use constable_core' crates/yah/yubaba/src/runtime/mod.rs crates/yah/yubaba/src/mesh/mod.rs  # both > 0")
//!
...
//! @arch:see(.yah/docs/working/W199-kamaji-universal-supervisor.md)
//! @yah:depends_on(R484-T2)
//! @yah:handoff("Two-constructor API now lives in kamaji-core. (1) Sibling: ~640 LoC moved from yubaba into crates/yah/kamaji/src/sibling.rs behind a new `sibling` cargo feature (pulls kamaji-proto as an optional dep). Yubaba's constable_client.rs became a 13-line re-export shim, then was deleted by R484-T5 along with this annotation's prior home.")
//! @yah:handoff("(2) Inlined: new crates/yah/kamaji/src/inlined.rs exposes Inlined::pick(&availability, &[Backend…], factory) -> Result<Arc<dyn Kamaji>, BackendUnavailable>. Walks the caller's preference list in order, returns the first backend whose probe came back available, invokes the factory exactly once with the chosen Backend tag. When none are available it returns BackendUnavailable for the LAST preference so callers who prefer Docker, else Containerd see the Containerd install hint when both are missing.")
//! @yah:handoff("Cargo wiring: kamaji-core gained `sibling = [\"dep:kamaji-proto\"]`; yubaba's kamaji-core dep is now `{ path = \"../kamaji\", features = [\"sibling\"] }`. Desktop (T6) leaves the sibling feature off.")
//! @yah:handoff("Verified at handoff: cargo test -p kamaji-core --all-features → 61 passed (was 50; +6 sibling +5 inlined). cargo test -p yubaba --lib --features containerd-integration,testing,docker-integration → 102 passed. cargo test -p yubaba --test integration_constable_client → 3 passed.")
//! @yah:verify("cargo test -p kamaji-core --all-features  # 61 passed (6 sibling + 5 inlined new)")
...
/// No-op `Kamaji` used as a placeholder in the smoke test tier.
///
/// The smoke tier provisions real Hetzner machines via cloud-init; the
/// `Kamaji` parameter in `test_cluster` is only used by the local tier.
...
/// method directly (rather than via the yubaba HTTP API), it will fail loudly.
pub struct DummyRuntime;

#[async_trait]
impl Kamaji for DummyRuntime {
