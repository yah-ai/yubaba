...
/// The argv for `litestream restore`, as a pure function so the shape can be
/// asserted without a litestream binary.
///
/// # The positional is the DB PATH, never the replica URL (R858-T5)
///
/// litestream 0.3.13 accepts `restore` in two mutually exclusive forms — with
/// `-config`, where the positional names a `dbs[].path` the config maps to a
/// replica, or without it, where the positional is a replica URL. Passing both
/// is refused outright:
///
/// ```text
/// ERROR failed to run error="cannot specify a replica URL and the -config flag"
/// ```
///
/// This function used to emit exactly that combination, so the restore leg
/// exited 1 on every node, every time, and `leader::on_became_leader` absorbed
/// it into a `warn!`. R591-T3 found three ways this path replicated nothing
/// while looking wired, R858 found a fourth (the `/etc` EROFS write); this was
/// the fifth, and it is the one that made the *restore* half inert even after
/// the other four were fixed. MEASURED on us-west-011 2026-09-08 against the
/// live `yah-headscale` bucket: the config+URL form exits 1, the config+path
/// form restores a seeded row and exits 0.
fn restore_args(db_path: &str) -> Vec<String> {
    vec![
        "restore".into(),
        "-if-replica-exists".into(),
        "-config".into(),
        LITESTREAM_CONFIG_PATH.into(),
        "-o".into(),
        db_path.into(),
        db_path.into(),
    ]
...
///
/// # A present DB is a skip, not a failure
///
/// litestream never overwrites: with the output path already on disk it exits 1
/// with `cannot restore, output path already exists` and leaves the file
/// untouched (measured, R858-T5 — both with and without `-o`). That is the
/// re-acquire case, where a node that served before still has its
/// `headscale.db`, and it is not an error — but letting it reach the `bail!`
/// below would make `on_became_leader`'s `warn!` fire on every ordinary
/// re-acquire, which is how a real restore failure gets lost. Skipping keeps a
/// nonzero exit meaning what an operator reads it as: the restore failed.
///
/// The consequence is deliberate and is the RPO this design already accepts —
/// a node with a local DB serves that DB, not the replica. Wiping
/// `headscale.db` is how an operator asks for the replica instead.
pub async fn restore(headscale_db: &Path, s3_url: &str) -> Result<()> {
    let (url_without_query, _) = split_s3_url(s3_url);

    if headscale_db.exists() {
        tracing::info!(
            "litestream restore skipped: {} already exists — serving the local DB (litestream \
    ...
    let args = restore_args(&headscale_db.to_string_lossy());

    let status = tokio::task::spawn_blocking(move || {
        std::process::Command::new("litestream").args(&args).status()
    })
    .await??;
...
mod tests {
    ...
    /// R858-T5. The bug this pins was invisible to every other test here: the
    /// argv looked plausible, and litestream is the only thing that knows
    /// `-config` and a replica URL are mutually exclusive. Asserting the
    /// positional is a *path* is the cheapest local stand-in for that rule.
    #[test]
    fn restore_passes_the_db_path_as_the_positional_not_the_replica_url() {
        let args = restore_args("/var/lib/yah-cloud/headscale/headscale.db");
        let positional = args.last().expect("argv is never empty");
        assert_eq!(
            positional, "/var/lib/yah-cloud/headscale/headscale.db",
            "the trailing positional must be the DB path — litestream 0.3.13 \
             refuses `-config` together with a replica URL"
        );
        assert!(
            !args.iter().any(|a| a.starts_with("s3://")),
            "no argument may be a replica URL while -config is passed: {args:?}"
        );
    }

    /// The restore and the replicate sidecar must read the same file, or the
    /// restore pulls from a replica the sidecar never writes.
    #[test]
    fn restore_reads_the_config_install_writes() {
        let args = restore_args("/tmp/headscale.db");
        let i = args
            .iter()
            .position(|a| a == "-config")
            .expect("-config is passed");
        assert_eq!(args[i + 1], LITESTREAM_CONFIG_PATH);
    }

    /// `-if-replica-exists` is what makes first bootstrap a no-op instead of a
    /// hard failure; losing it turns every fresh mesh into a start-up error.
    #[test]
    fn restore_is_a_no_op_when_the_replica_is_empty() {
        assert!(restore_args("/tmp/headscale.db")
            .iter()
            .any(|a| a == "-if-replica-exists"));
    }
