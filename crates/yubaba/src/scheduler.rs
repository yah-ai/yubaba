...
//!
//! @yah:ticket(R523-F2, "Wire transit-cost.toml into scheduler placement scoring (the F16 half R330-F17 left undone)")
//! @yah:at(2026-09-13T20:28:06Z)
//! @yah:status(open)
//! @yah:assignee(agent:bundle-anthropic-miravel)
//! @yah:parent(R523)
//! @yah:gotcha("Live multi-region fleet verification of R330-F17's actual routing behavior (dialing from a real us-west/eu-west operator box, per that ticket's Verification section) was NOT done — this session had no live multi-region fleet access. All of F17's verify bullets are covered by unit/integration tests against the real production code paths (ServiceRecordMeshState, RouteTables, MeshDirectory) with synthetic peer data instead. An operator should run one real end-to-end check (deploy a service with HA instances across two regions, confirm nearest-first resolution from each side) before relying on this in production.")
//! @yah:assumes("Placement-time provider-aware scoring is a real, separate design decision from call-time routing (different failure modes: a bad placement choice is corrected by a re-schedule, a bad call-time choice is corrected on the next resolution) and should NOT reuse route_score::pick_nearest verbatim without checking whether scheduler.rs's existing eligibility/scoring shape (NodeEligibility, TenantSnapshot in scheduler.rs) wants a different integration point.")
//! @arch:see(.yah/docs/working/W059-almanac-release-feed.md)
//! @yah:next("scheduler.rs already has NodeEligibility.region and TenantSnapshot.region (R734-era) feeding placement, but no provider-aware cost term — provider IS already available per-node via raft's MemberInfo.provider (R859-F2), just unused in scoring. Add an egress-cost term using RouteTables::transit_cost, analogous to how mesh_resolve now uses route_score::score for call-time routing.")
//! @yah:verify("Unit tests mirroring route_score::tests::default_weights_favor_same_provider_over_same_region, but asserting the SCHEDULER's placement choice rather than the router's.")
