...
//! @yah:handoff("GAP 1 (CAPABILITY DRIFT PLACEMENT CANNOT SEE) IS FILLED — the producer, not the predicate. R858-T3's `appliance_ownership.rs` had already defined `NativeExecCapability::{Unknown,Present,Absent}` and named it \"R858-T4's seam\", with exactly one live call site passing the permissive `Unknown` placeholder at leader.rs:355. That placeholder is now a real probe and nothing about `judge_appliance_candidate` changed, which is what T3 asked for. THE WIRE, deliberately the cheapest shape that is still honest: `YubabaToKamaji::Capabilities { request_id }` and `KamajiToYubaba::CapabilitiesReport { request_id, capabilities: NodeCapabilities { native_exec, native_exec_dir } }`, both APPENDED LAST so every pre-existing postcard variant index is untouched and NO ProtocolVersion bump is needed — the same treatment `GracefulUpgrade` and `DeployStatus` had. version.rs's V2/V4/V5/V6 bumps do not apply here because those all added a field to an EXISTING variant, which is positional and therefore breaking; V6's note records that the `#[serde(default)]` theory cost a debugging cycle, and I did not re-learn it. I ALSO ADDED THE VARIANT TO `reply_request_id()` — that function's doc comment is explicit (R746-B11) that a new reply variant left unclassified there falls into the push arm, is dropped as an unconsumed push, and parks its caller on a oneshot forever, which is exactly what happened to `DeployStatusResult`. The hand-written correlation test now carries `CapabilitiesReport` too. KAMAJI ANSWERS FROM `ctx.native`, the same `Option<Arc<NativeRuntime>>` that `deploy_native_exec` dispatches on, so the capability a scheduler reads and the capability a deploy exercises cannot disagree — deriving it from build features or CLI args would be a second source of truth that drifts exactly when it matters. `NativeRuntime::exec_dir()` was added to expose the staging dir. YUBABA'S PROBE (`leader::probe_native_exec`) checks BOTH measured gaps: kamaji's native backend via the new request, and `<headscale_dir>/headscale` on disk, because a native workload pulls nothing and a node with the flag but no binary refuses just as hard. EVERY FAILURE TO *LEARN* IS `Unknown`, NEVER `Absent` — no sibling client, an older kamaji that does not know the variant, a transient socket error. Reading \"I could not ask\" as \"cannot run it\" would make every not-yet-rolled node ineligible the moment this shipped: the 2026-09-03 outage reproduced from the other side, and mid-roll that is most of the fleet. The old-kamaji case logs at `debug!` not `warn!` for the same reason — during a normal rolling upgrade it is the expected answer on every tick, and a warning that fires constantly trains operators to ignore the channel R858-T3 just made loud.")
//! @yah:handoff("GAP 1 (CAPABILITY DRIFT PLACEMENT CANNOT SEE) IS FILLED — the producer, not the predicate. R858-T3's `appliance_ownership.rs` had already defined `NativeExecCapability::{Unknown,Present,Absent}` and named it \"R858-T4's seam\", with exactly one live call site passing the permissive `Unknown` placeholder at leader.rs:355. That placeholder is now a real probe and nothing about `judge_appliance_candidate` changed, which is what T3 asked for. THE WIRE, and it is deliberately the cheapest shape that is still honest: `YubabaToKamaji::Capabilities { request_id }` and `KamajiToYubaba::CapabilitiesReport { request_id, capabilities: NodeCapabilities { native_exec, native_exec_dir } }`, both APPENDED LAST so every pre-existing postcard variant index is untouched and NO ProtocolVersion bump is needed — the same treatment `GracefulUpgrade` and `DeployStatus` already had, and the reason version.rs's V2/V4/V5/V6 notes do not apply (those all added a field to an EXISTING variant, which is positional and therefore breaking; V6's note records that the `#[serde(default)]` theory cost a debugging cycle, and I did not re-learn it). I ALSO ADDED THE VARIANT TO `reply_request_id()` — R746-B11's doc comment on that function is explicit that a new reply variant which is not classified there falls into the push arm, gets dropped as an unconsumed push, and parks its caller on a oneshot forever; that is exactly what happened to `DeployStatusResult`. The hand-written correlation test now carries `CapabilitiesReport` too. KAMAJI ANSWERS FROM `ctx.native`, the same `Option<Arc<NativeRuntime>>` that `deploy_native_exec` dispatches on, so the capability a scheduler reads and the capability a deploy exercises cannot disagree — deriving it from build features or CLI args would be a second source of truth that drifts exactly when it matters. `NativeRuntime::exec_dir()` was added to expose the staging dir. YUBABA'S PROBE (`leader::probe_native_exec`) checks BOTH measured gaps: kamaji's native backend via the new request, and `<headscale_dir>/headscale` on disk, because a native workload pulls nothing and a node with the flag but no binary refuses just as hard. EVERY FAILURE TO *LEARN* IS `Unknown`, NEVER `Absent` — no sibling client, an older kamaji that does not know the variant, a transient socket error. Reading \"I could not ask\" as \"cannot run it\" would make every not-yet-rolled node ineligible the moment this shipped, which is the 2026-09-03 outage reproduced from the other side and, mid-roll, would be most of the fleet. The old-kamaji case logs at `debug!` rather than `warn!` for the same reason: during a normal rolling upgrade it is the expected answer on every tick, and a warning that fires constantly trains operators to ignore the channel R858-T3 just made loud.")
//! @yah:handoff("GAP 1 (CAPABILITY DRIFT PLACEMENT CANNOT SEE) IS FILLED — the producer, not the predicate. R858-T3's `appliance_ownership.rs` had already defined `NativeExecCapability::{Unknown,Present,Absent}` and named it \"R858-T4's seam\", with exactly one live call site passing the permissive `Unknown` placeholder at leader.rs:355. That placeholder is now a real probe and nothing about `judge_appliance_candidate` changed, which is what T3 asked for. THE WIRE, deliberately the cheapest shape that is still honest: `YubabaToKamaji::Capabilities { request_id }` and `KamajiToYubaba::CapabilitiesReport { request_id, capabilities: NodeCapabilities { native_exec, native_exec_dir } }`, both APPENDED LAST so every pre-existing postcard variant index is untouched and NO ProtocolVersion bump is needed — the same treatment `GracefulUpgrade` and `DeployStatus` had. version.rs's V2/V4/V5/V6 bumps do not apply because those all added a field to an EXISTING variant, which is positional and therefore breaking; V6's note records that the `#[serde(default)]` theory cost a debugging cycle, and I did not re-learn it. I ALSO ADDED THE VARIANT TO `reply_request_id()` — that function's doc comment is explicit (R746-B11) that a new reply variant left unclassified there falls into the push arm, is dropped as an unconsumed push, and parks its caller on a oneshot forever, which is exactly what happened to `DeployStatusResult`. The hand-written correlation test carries `CapabilitiesReport` now too. KAMAJI ANSWERS FROM `ctx.native`, the same `Option<Arc<NativeRuntime>>` that `deploy_native_exec` dispatches on, so the capability a scheduler reads and the capability a deploy exercises cannot disagree — deriving it from build features or CLI args would be a second source of truth that drifts exactly when it matters. `NativeRuntime::exec_dir()` was added to expose the staging dir. YUBABA'S PROBE (`leader::probe_native_exec`) checks BOTH measured gaps: kamaji's native backend via the new request, and `<headscale_dir>/headscale` on disk, because a native workload pulls nothing and a node with the flag but no binary refuses just as hard. EVERY FAILURE TO *LEARN* IS `Unknown`, NEVER `Absent` — no sibling client, an older kamaji that does not know the variant, a transient socket error. Reading \"I could not ask\" as \"cannot run it\" would make every not-yet-rolled node ineligible the moment this shipped: the 2026-09-03 outage reproduced from the other side, and mid-roll that is most of the fleet. The old-kamaji case logs at `debug!` not `warn!` for the same reason — during a normal rolling upgrade it is the expected answer on every tick, and a warning that fires constantly trains operators to ignore the channel R858-T3 just made loud.")
...
//! @yah:next("WHAT IS LEFT ON THIS TICKET, and the first item is the one that actually converts the operator's risk budget into HA. (1) NOTHING HAS ROLLED. Both halves reach nodes only via a yubaba release + scripts/roll-node.sh, and the mirror.yml half only affects nodes PROVISIONED after it — an existing node never re-runs cloud-init. So us-south-001 still has no `--native-exec-dir` and no headscale binary, and a leadership move off west still ends with the appliance running nowhere. Closing that on the LIVE south is two operator actions, both cheap and both on a follower: roll the node so its kamaji.service carries the flag it has been missing (app/yah/cli/resources/kamaji.service:49 already has it — south is drift from the shipped unit, not a missing feature), and place the binary. Until that happens this ticket has made the failure LOUD and EARLY but has not made the failover WORK. (2) NO UNIT TEST COVERS `probe_native_exec` — stated plainly rather than implied by a green suite. It is an async fn over a live `KamajiSibling`, and the seam that would make it testable (injecting a capability source) does not exist; the three-way mapping it implements is simple and the branches are individually obvious, but \"685 passed\" is not evidence about this function. Whoever adds the injection point gets three cheap tests: no client -> Unknown, Err -> Unknown, native_exec=false -> Absent. (3) The `native_exec_dir` field is carried and only logged today. It exists so the binary check can eventually ask kamaji where it would exec FROM rather than assuming `<headscale_dir>`; the two are the same path on this fleet and are not the same idea.")
...
//! @yah:next("TWO OPERATOR ACTIONS TURN DECAPITATION INTO FAILOVER, both cheap and both on a FOLLOWER rather than on the coordinator, which is why this is the safest remaining step on the whole relay. (1) Roll us-south-001 so its kamaji.service carries `--native-exec-dir` — app/yah/cli/resources/kamaji.service:49 has had the flag all along, so south is per-node DRIFT from the shipped unit, not a missing feature. (2) Put the headscale binary at /var/lib/yah-cloud/headscale/headscale on south (the mirror.yml block added this pass is the recipe: v0.23.0, sha256 d9193dad4b070b9b3f6d54c8f14366952944b6e917672c0bc1dfd8f5491287a7 for amd64). Restarting kamaji on south restarts south's kamaji-supervised workloads but does NOT touch raft leadership or the appliance, which lives on west — so unlike almost everything else on this relay it is not an outage risk to the mesh. AFTERWARDS, the acceptance check is not \"the test suite is green\" but `probe_native_exec` returning Present on south: with the roll in place, a `POST /raft/transfer-leader` should produce a real failover instead of the Sept 3 chain. That is R858-T8's rehearsal and it should not be attempted before both actions above are done and verified on the box.")
