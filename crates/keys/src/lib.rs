...
//! @yah:handoff("Today the workspace has two parallel secret stores: the CLI/cloud uses crates/yah/keys (AES-256-GCM file at ProjectDirs::data_dir(), works headless/over-SSH/on-Linux-warden) while the desktop uses app/yah/desktop/src/api_keys.rs (macOS Keychain via the keyring crate). Same machine, two vaults, inconsistent slot naming (CLI 'hetzner-api-token' vs desktop 'hetzner'). User asked to bridge them with the encrypted-file backend as the canonical store so dev-machine UX (desktop) and headless infra (warden, agentd, ssh'd camps) share one source of truth. F9 already lifted KeysStore into a shared crate and explicitly named this as its R040-Tx follow-up.")
...
//! @yah:phase(P4)
//! @yah:parent(R043)
//! @yah:next("Lands after R043-F1/F2/F3 stabilize the unified vault. Use case: operator has creds on desktop, needs them on a remote yah-camp (Path 2 SSH or Path 3 warden) without re-entering everything.")
...
//! @yah:next("Warden integration is a follow-up: warden as credential broker (either operator-driven 'yah cloud machine attach' triggers vault transfer, or warden pulls from cluster-shared encrypted store). Punt until R040-F20 (warden openraft) lands; export/import is the building block.")
