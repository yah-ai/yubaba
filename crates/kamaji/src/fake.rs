//! `runtime::fake` — in-memory `ContainerRuntime` for yubaba unit tests.
//!
//! ## Design
//!
...
mod tests {
    ...
    async fn mark_restarting_drives_status_for_crash_loop_fixture() {
        ...
        let rt = FakeRuntime::new();
        rt.mark_restarting("yah-yubaba", 2, 3, 1_700_000_000_000);

        let state = rt
            .get_workload(&MeshIdent("yah-yubaba".to_string()))
            .await
            .unwrap()
